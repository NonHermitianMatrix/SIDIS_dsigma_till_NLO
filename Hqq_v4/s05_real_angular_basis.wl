...
previousHash = Hash[{FileHash[previousSource, "SHA256"],
  FileHash[FileNameJoin[{root, "s03_result.wl"}], "SHA256"]}, "SHA256"];
previousCache = FileNameJoin[{root, "s05_previous_14689280", "s05_cache", IntegerString[previousHash, 16]}];
...
mergeBases[key_] := Module[{aliases, pieces, denominators, geometries, matches, terms, groups,
  ...
  terms = Flatten[Table[
    Table[remapped = row;
      remapped[[1]] = First[FirstPosition[denominators, piece["Denominators"][[row[[1]]]], Missing["NotFound"], {1}]];
      remapped[[2]] = First[FirstPosition[denominators, piece["Denominators"][[row[[2]]]], Missing["NotFound"], {1}]];
      gate["remapped angular term preserves its pair denominators",
        denominators[[remapped[[{1, 2}]]]] === piece["Denominators"][[row[[{1, 2}]]]]];
  ...
  value = <|"Denominators" -> denominators, "Geometry" -> geometries, "Terms" -> terms,
    "VerifiedPairTasks" -> aliases, "ReconstructionProof" -> "S03 pair sum, each pair basis, each remapped denominator identity, each merged coefficient"|>;
