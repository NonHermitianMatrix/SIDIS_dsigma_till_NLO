...
bounded[work_, label_] := MemoryConstrained[TimeConstrained[work, 900,
  Print["FAIL: time bound ", label]; Quit[2]], 2*1024^3,
  Print["FAIL: memory bound ", label]; Quit[3]];
...
(* Fixed exact seeds; points are selected by physical support and branch only. *)
makePoint[q_, xx_, ratio_, label_] := Module[{base, bvalue, mass, tvalue, rules, sign, support},
...
If[workers > 0,
  ...
  DistributeDefinitions[gate, bounded, atomicPut, number, endpoint, pack, runPoint,
...
results = If[workers > 0, ParallelMap[runPoint, points, Method -> "FinestGrained"], runPoint /@ points];
CloseKernels[];
