...
for projection in ["Pg", "Ppp"]:
    for case in "ABC":
        name = f"NLO/{projection}/fchn6{case}.py"
...
for point in data["points"]:
    ...
    case = point["case"]
    raw = [functions[projection, case, "regular"](**args) for projection in ["Pg", "Ppp"]]
    ...
    augmented_raw = [v + functions[projection, case, "plus1B"](**args, B=p["s"])/p["s23"]
                     + functions[projection, case, "plus2B"](**args, B=p["s"])*math.log(p["s23"])/p["s23"]
    ...
    for representation, values in [("regular_only", hats), ("all_supplied_coefficients", augmented_hats)]:
        for label, published, local in zip(["F1hat", "F2hat"], values, point["local_hats"]):
            ...
            rows.append({"case": case, "parameters": p, "branch_s_plus_t": point["branch_s_plus_t"],
    ...
    print(case, p, "all-coefficient relative differences", [r["relative_difference"] for r in rows[-2:]], flush=True)
...
report = {"reference_commit": manifest["commit"], "production_sha256": manifest["production_sha256"],
          ...
          "distribution_policy": "The pinned driver includes channel 6 but restricts its plus selection to channels below 4. This check calls all channel-6 coefficient functions directly. Wolfram verifies their plus-coefficient endpoints before the complete ordinary density is assembled. The production result is unchanged."}
(BASE / "s03_result.json").write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
lines = ["# Hqqprime BigTMD comparison", "", "This check compares the supplied Hqqprime coefficient functions directly. The pinned public driver includes channel 6 but restricts its plus selection to channels below 4.",
...
for row in rows:
    p = row["parameters"]
    lines.append(f"| {p['Q2']} | {p['s']} | {p['s23']} | {p['t']} | {p['mu2']} | {row['case']} | {row['representation']} | {row['hat']} | {row['BigTMD_minus_local']:.9g} | {row['pass']} |")
