...
ClearAll[
  assert, fileSHA256, relativeDifference, machineRealQ,
...
parseScientificDecimal[token_String] := Module[
  ...
  assert[
    MemberQ[{1, 2}, Length[exponentParts]],
  ...
  assert[
    MemberQ[{1, 2}, Length[decimalParts]],
...
expectedEvaluatorHash =
  "ab1b2cb3af9026055c70d5feb309da3d4ffe37ffb3c2526d7cadcf16c48309bd";
expectedInputCSVHash =
  "6f64a12a37016138a92eab48fef98e3018504800b2b5f5fea9ec803a9e44a7ab";
expectedInputMetadataHash =
  "78881497b9341b00d0346781657d795f8459bdc4d02ca58ebee379ec12de0b1d";
expectedParameterCardHash =
  "55bb009a781370ab3fdfb523be00d0c8f68ffec442d12eb41b8d2da50307cf64";

assert[
  fileSHA256[evaluatorPath] === expectedEvaluatorHash,
  "accepted S09 evaluator hash is invalid"
];
assert[
  fileSHA256[inputCSVPath] === expectedInputCSVHash,
  "accepted S10 CSV hash is invalid"
];
assert[
  fileSHA256[inputMetadataPath] === expectedInputMetadataHash,
  "accepted S10 metadata hash is invalid"
];
assert[
  fileSHA256[parameterCardPath] === expectedParameterCardHash,
...
assert[
  AssociationQ[evaluator] && evaluator["Status"] === "Complete" &&
    evaluator["Stage"] === "HqgFourDimensionalEvaluator-v1" &&
...
assert[
  AssociationQ[metadata] && metadata["status"] === "complete" &&
    metadata["stage"] === "HqgFiniteCutMadGraphSample-v1" &&
    metadata["csv_sha256"] === expectedInputCSVHash &&
    metadata["local_and_generated_final_order"] ===
      {"g(k1)", "u(k2)", "g(k3)"},
  "S10 metadata did not load as the accepted Hqg stage"
];
...
assert[
  Abs[N[alphaS] - metadata["alpha_s"]] < 10^-15,
...
assert[
  Dimensions[unitProjectionValues] === {Length[invariantRows], 2} &&
    MatrixQ[unitProjectionValues, machineRealQ],
  "compiled evaluator did not return a machine-real array"
];
...
assert[
  VectorQ[localValues, machineRealQ] && Min[localValues] > 0.0,
...
assert[
  ! FileExistsQ[outputCSVTemporary],
...
checks = <|
  ...
  "physical_charge_not_reapplied" -> True,
  "identity_or_symmetry_factor_not_reapplied" -> True,
...
|>;
summary = <|
  "stage" -> "HqgFiniteCutPointwiseComparison-v1",
  "status" -> "complete",
...
assert[
  ! FileExistsQ[outputJSONTemporary],
