...
ClearAll[
  assert, atomicPutAssociation, relativeDifference, machineRealQ,
...
machineRealQ[value_] := Head[value] === Real && MachineNumberQ[value];
...
atomicPutAssociation[association_Association, path_String] := Module[
  ...
  assert[
    AssociationQ[reloaded] && reloaded["Status"] === "Complete" &&
      reloaded["Stage"] === "HqgFourDimensionalEvaluator-v1" &&
...
expectedHashes = <|
  ...
  projectedPointPath ->
    "518e1b940c177027f52c82e7d2ebed10d6551e37b5d89aa7223939fbcce929f9",
  azimuthalValidationPath ->
    "a04e67cca4d71e31f1c45fcc3a6a0c3c0fce1c92b650328aaffc5fdca3213a70",
  parameterCardPath ->
...
Print["S09_STAGE: loading accepted Hqg projections"];
...
assert[
  AssociationQ[projectedPoint] && projectedPoint["status"] === "complete" &&
    projectedPoint["stage"] === "HqgLocalTensorAndProjectionValidation-v1" &&
...
assert[
  AssociationQ[azimuthalValidation] &&
    ...
    azimuthalValidation["stage"] ===
      "HqgMadGraphAzimuthalAverageValidation-v1" &&
...
assert[
  ...
  "copied Hqg normalization or observed-gluon lineage is invalid"
];
...
(* Derive every independent scalar product from the definitions of the six
   three-body invariants; no propagator invariant is transcribed. *)
...
Print["S09_STAGE: mapping Hqg Pg/PPP payloads to invariants"];
rawUnitProjections = AssociationMap[
  mapDenominators[projections[#]] /. unitFourDimensionalRules &,
  projectorKeys
];
rawComplexCounts = AssociationMap[
  Count[rawUnitProjections[#], _Complex, Infinity] &,
  projectorKeys
];
unitProjections = AssociationMap[
  Function[projectorName,
    Module[{realPart, imaginaryPart},
      realPart = ComplexExpand[Re[rawUnitProjections[projectorName]]];
      imaginaryPart = ComplexExpand[Im[rawUnitProjections[projectorName]]];
      assert[
        TrueQ[PossibleZeroQ[imaginaryPart]],
        projectorName <> " has a nonzero symbolic imaginary part"
      ];
      assert[
        TrueQ[PossibleZeroQ[
          ComplexExpand[rawUnitProjections[projectorName] - realPart]
        ]],
        projectorName <> " failed exact real-part reconstruction"
      ];
      assert[
        FreeQ[realPart, _Complex],
        projectorName <> " retains an explicit complex atom after realification"
      ];
      realPart
    ]
  ],
...
unitProjectionLeafCounts = AssociationMap[LeafCount[unitProjections[#]] &, projectorKeys];
physicalProjectionLeafCounts = AssociationMap[
  LeafCount[physicalUnitProjections[#]] &,
  projectorKeys
];
Print[
  "S09_RAW_COMPLEX_COUNTS=",
  InputForm[rawComplexCounts]
];
Print[
...
assert[
  VectorQ[benchmarkUnit, machineRealQ] && Length[benchmarkUnit] === 2,
  "the compiled evaluator did not return two machine-real values"
];
...
checks = <|
  ...
  "BothProjectorPayloadsMapped" -> True,
  "BothProjectorImaginaryPartsExactlyZero" -> True,
  "StoredProjectorsContainNoComplexAtoms" -> True,
  "PhysicalChargeDerivedFromCopiedS01" -> True,
...
output = <|
  "Status" -> "Complete",
  "Stage" -> "HqgFourDimensionalEvaluator-v1",
  "ProgramSHA256" ->
  ...
  "PhysicalRepresentative" -> <|
    "Incoming" -> "u",
    "Fragmenting" -> "g(k1)",
    "ReferenceCharge" -> referenceCharge,
    "AmplitudeStripFactor" -> stripFactor,
    "ChargeSquared" -> chargeSquared
  |>,
  "CouplingConvention" ->
  ...
  "RawComplexAtomCounts" -> rawComplexCounts,
  "UnitCouplingBareProjections" -> unitProjections,
  "UnitCouplingPhysicalProjections" -> physicalUnitProjections,
  "LeafCounts" -> <|
    "Bare" -> unitProjectionLeafCounts,
