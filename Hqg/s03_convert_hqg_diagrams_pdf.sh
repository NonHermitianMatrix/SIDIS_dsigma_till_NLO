...
# Convert the four Hqg S02 PostScript diagram sheets into a validated PDF.
# The Hqq-bundled Ghostscript tree is reused only as a generic runtime.
...
s03_output_pdf="s03_hqg_feynman_diagrams.pdf"
...
test -d "$s03_pages_directory" || \
  s03_fail "The Hqg S02 PostScript page directory is missing."

shopt -s nullglob
s03_pages=("$s03_pages_directory"/s02_page_*.ps)
shopt -u nullglob

if [[ "${#s03_pages[@]}" -ne 4 ]]; then
  s03_fail "Expected exactly four Hqg S02 PostScript pages."
fi

for s03_page in "${s03_pages[@]}"; do
  test -s "$s03_page" || \
    s03_fail "An Hqg S02 PostScript page is missing or empty."
  IFS= read -r s03_header < "$s03_page"
  [[ "$s03_header" == '%!PS-Adobe-'* ]] || \
    s03_fail "An Hqg S02 page lacks the required PostScript header."
done
...
printf 'S03_STAGE: validating %s Hqg PostScript input pages\n' "${#s03_pages[@]}"
"$s03_gs_binary" \
  -q -dSAFER -dBATCH -dNOPAUSE \
  -sDEVICE=nullpage \
  "${s03_pages[@]}"

printf 'S03_STAGE: combining Hqg diagram pages into PDF\n'
"$s03_gs_binary" \
  -q -dSAFER -dBATCH -dNOPAUSE \
  -sDEVICE=pdfwrite \
...
test -s "$s03_output_pdf" || \
  s03_fail "The finalized Hqg PDF is missing or empty."
