...
LOCAL_POINT = ROOT / "s07_projected_local_point.json"
...
EXPECTED_SHA256 = {
    ...
    LOCAL_POINT: "8f8e8319301efb68cadf8226907ed4cce71df51660e5abe9e27e47142d754ca5",
}
...
def main() -> None:
    ...
    if local_point.get("stage") != "HqqbarProjectedLocalPoint-v1":
        raise RuntimeError("unexpected projected-local stage")
    ...
    alpha_s = float(local_point["alpha_s"])
    ...
    madgraph_average = math.fsum(values) / len(values)
    local_value = float(local_point["local_matrix_element"])
    average_relative_difference = relative_difference(madgraph_average, local_value)
    first_angle_reference = float(
        local_point["single_orientation_madgraph_matrix_element"]
    )
    first_angle_relative_difference = relative_difference(values[0], first_angle_reference)
    ...
    output = {
        "stage": "HqqbarMadGraphAzimuthalAverageValidation-v1",
        "status": "complete",
        "azimuth_angles_radians": angles,
        "madgraph_matrix_elements": values,
    ...
    print(f"S08_MADGRAPH_VALUES={values}")
