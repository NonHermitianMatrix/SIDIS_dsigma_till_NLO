module hqqbar_madgraph_c_bridge
  use, intrinsic :: iso_c_binding
  implicit none

contains

  subroutine hqqbar_mg_init(c_path) bind(C, name="hqqbar_mg_init")
    ...
    if (.not. found_terminator) then
      error stop "HQQBAR_MG_FATAL: parameter-card path exceeds 511 bytes"
    end if

    call setparamlog(.false.)
    call setpara(fortran_path)
  end subroutine hqqbar_mg_init

  subroutine hqqbar_mg_eval(momentum, alpha_s, answer) &
      bind(C, name="hqqbar_mg_eval")
    ...
    pdgs = [11_c_int, 2_c_int, 11_c_int, 2_c_int, 2_c_int, -2_c_int]
    process_id = 1_c_int
  ...
  end subroutine hqqbar_mg_eval

  subroutine hqqbar_mg_metadata(pdgs, identity_factor) &
      bind(C, name="hqqbar_mg_metadata")
    integer(c_int), intent(out) :: pdgs(6)
    integer(c_int), intent(out) :: identity_factor

    pdgs = [11_c_int, 2_c_int, 11_c_int, 2_c_int, 2_c_int, -2_c_int]
    identity_factor = 24_c_int
  end subroutine hqqbar_mg_metadata

end module hqqbar_madgraph_c_bridge
