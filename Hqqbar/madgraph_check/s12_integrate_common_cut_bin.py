...
EXPECTED_SHA256 = {
    S10_METADATA: "086a3809c90d24f66aea6f29648c950a37fd9955fe9987f3e9368c6f6020c824",
    S11_POINTS: "0afe3400b01ea9e7598f4507727d1d15b5a5ee4d9649482c2e13d1893845e34f",
    S11_SUMMARY: "4e157028c67ac86c34ac53f81dda15c9b30a8a4f90806862ded4c916b101224b",
}
...
def mean_variance_standard_error_with_rejected_zeros(
    ...
    variance = total_trials / (total_trials - 1.0) * (second_moment - mean * mean)
...
def main() -> None:
    ...
    if metadata.get("status") != "complete" or pointwise_summary.get("status") != "complete":
        raise RuntimeError("an input stage is not complete")
    ...
    with S11_POINTS.open(newline="", encoding="utf-8") as stream:
        ...
        expected_columns = {
            ...
            "madgraph_azimuthal_average", "local_projected_matrix_element",
            "signed_difference", "relative_difference",
        }
    ...
    recomputed_differences = [
        local - madgraph for local, madgraph in zip(local_values, madgraph_values)
    ]
    maximum_difference_reconstruction_residual = max(
        abs(recomputed - stored)
        for recomputed, stored in zip(recomputed_differences, stored_signed_differences)
    )
    ...
    mg_mean, mg_variance, mg_mean_error = (
        mean_variance_standard_error_with_rejected_zeros(madgraph_values, total_trials)
    )
    local_mean, local_variance, local_mean_error = (
        mean_variance_standard_error_with_rejected_zeros(local_values, total_trials)
    )
    difference_mean, difference_variance, difference_mean_error = (
        mean_variance_standard_error_with_rejected_zeros(
            recomputed_differences, total_trials
        )
    )

    cross_moment = math.fsum(
        madgraph * local for madgraph, local in zip(madgraph_values, local_values)
    ) / total_trials
    ...
    integrated_relative_difference = abs(difference_cross_section_pb) / max(
        abs(mg_cross_section_pb), abs(local_cross_section_pb)
    )
    ...
    output = {
        "stage": "HqqbarFiniteCutIntegratedComparison-v1",
        "status": "complete",
        ...
        "local_copied_s07_cross_section_pb": local_cross_section_pb,
        ...
        "difference_relative_to_madgraph_mc_error": difference_relative_to_mg_mc_error,
    ...
    print(f"S12_INTEGRATED_RELATIVE_DIFFERENCE={integrated_relative_difference:.17e}")
