C     PY ((11, 2), (2, 11, 21, 21)) : (11, 2, 11, 2, 21, 21) # M0_ 1
C     PY ((11, 2), (-2, 2, 2, 11)) : (11, 2, 11, 2, 2, -2) # M1_ 2
C     PY ((11, 2), (-1, 1, 2, 11)) : (11, 2, 11, 2, 1, -1) # M2_ 3
...
CF2PY double precision, intent(in) :: SCALE2
     ...
     $ .AND.2.EQ.PDGS(4).AND.21.EQ.PDGS(5).AND.21.EQ.PDGS(6)
     $ .AND.(PROCID.LE.0.OR.PROCID.EQ.1)) THEN  ! 0
        CALL M0_SMATRIXHEL(P, NHEL, ANS)
      ELSE IF(11.EQ.PDGS(1).AND.2.EQ.PDGS(2).AND.11.EQ.PDGS(3)
     $ .AND.2.EQ.PDGS(4).AND.2.EQ.PDGS(5).AND.-2.EQ.PDGS(6)
     $ .AND.(PROCID.LE.0.OR.PROCID.EQ.2)) THEN  ! 1
        CALL M1_SMATRIXHEL(P, NHEL, ANS)
      ELSE IF(11.EQ.PDGS(1).AND.2.EQ.PDGS(2).AND.11.EQ.PDGS(3)
     $ .AND.2.EQ.PDGS(4).AND.1.EQ.PDGS(5).AND.-1.EQ.PDGS(6)
     $ .AND.(PROCID.LE.0.OR.PROCID.EQ.3)) THEN  ! 2
...
CF2PY INTEGER, intent(out) :: ALLPROC(3)
      ...
      DATA PDGS/ 11,11,11,2,2,2,11,11,11,2,2,2,21,2,1,21,-2,-1 /
      DATA PIDS/ 1,2,3 /
      PDG = PDGS
      ALLPROC = PIDS
      RETURN
...
CF2PY integer, intent(in) :: new_value
      ...
      RETURN
      END
      SUBROUTINE F77_M1_GET_NHEL_ENTRY(NHEL)
      INTEGER M1_NHEL(6,64), NHEL(6,64)
      COMMON/M1_PROCESS_NHEL/M1_NHEL
      NHEL(:,:) = M1_NHEL(:,:)
      RETURN
      END
      SUBROUTINE F77_M2_GET_NHEL_ENTRY(NHEL)
      INTEGER M2_NHEL(6,64), NHEL(6,64)
