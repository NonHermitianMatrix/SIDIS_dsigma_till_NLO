...
def source_lines(source):
    return source.encode("utf-8").splitlines(keepends=True)


def to_wl(node, source):
    if isinstance(node, ast.Constant) and type(node.value) in (int, float):
        assert node.lineno == node.end_lineno, "Numeric literals occupy one line"
        token = source_lines(source)[node.lineno-1][node.col_offset:node.end_col_offset].decode("utf-8")
        assert ast.literal_eval(token) == node.value, "Literal extraction preserves the parsed token"
        return exact_number(token)
...
def expressions(source, names):
    ...
    for name in names:
        ...
        print("IMPORTED", name, len(result[name]), "characters", flush=True)
