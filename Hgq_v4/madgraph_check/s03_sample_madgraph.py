"""Reconstruct Hgq Pg/Ppp from fresh photon-only MadGraph matrix elements."""
...
def build_process(kind):
    ...
    subprocess.run(["make", "-j1"], cwd=process / "Source", check=True)
    ...
    gate(nexternal == len(incoming+outgoing) and incoming == ["e-", "g"], kind + " leg ordering")
    gate(Counter(outgoing) == Counter(["e-", "u", "u~"] + (["g"] if kind == "real" else [])),
         kind + " final species")
    counts = Counter(outgoing)
    symmetry = math.prod(math.factorial(count) for count in counts.values())
    initial_average = math.prod(len(particles[name]["helicities"]) * abs(particles[name]["color"]) for name in incoming)
    ...
    gate(tagged_conversion == tags, kind + " tagged-quark normalization")
    ...
    bridge.write_text("""      SUBROUTINE HGQ_INIT(EEOUT,GSOUT,QUOUT,QEOUT)
...
""")
    ...
    library.hgq_init.argtypes = [ct.POINTER(ct.c_double)] * 4
    library.hgq_matrix.argtypes = [ct.POINTER(ct.c_double), ct.POINTER(ct.c_double)]
    constants = [ct.c_double() for _ in range(4)]
    previous = Path.cwd()
    os.chdir(directory)
    try:
        library.hgq_init(*(ct.byref(v) for v in constants))
    finally:
        os.chdir(previous)
    ...
    def evaluate(momenta):
        ...
        answer = ct.c_double()
        library.hgq_matrix(array.ctypes.data_as(ct.POINTER(ct.c_double)), ct.byref(answer))
    ...
    info = {"Process": description, "Incoming": incoming, "Outgoing": outgoing,
            ...
            "FinalSymmetryDenominator": symmetry, "QuarkTags": tags,
...
def sample(kind, evaluate, info):
    ...
    indices = {"lin": incoming.index("e-"), "p": incoming.index("g"),
               "lout": nin+outgoing.index("e-"), "k1": nin+outgoing.index("u"), "k2": nin+outgoing.index("u~")}
    ...
    while len(rows) < POINTS_PER_PROCESS:
        ...
        hadrons = {"p": p.tolist(), "q": q.tolist(), "k1": momenta[indices["k1"]].tolist(),
                   "k2": momenta[indices["k2"]].tolist()}
        if gluons:
            gate(len(gluons) == 1, "unique real recoil gluon")
            hadrons["k3"] = momenta[gluons[0]].tolist()
...
result = {"Channel": "Hgq", "SourceSHA256": sha(Path(__file__)), "InputManifestSHA256": sha(ROOT / "s01_result.json"),
...
print("HGQ_MADGRAPH_S03_SUCCESS", flush=True)
