...
priorSources = FileNames["s11_source_*.wl", cache];
acceptedHashes = Prepend[Table[Hash[Prepend[FileHash[FileNameJoin[{root, #}], "SHA256"] & /@
  {"s07_result.wl", "s08_result.wl"}, FileHash[source, "SHA256"]], "SHA256"],
  {source, priorSources}], inputHash];
...
softRows[expression_, kappa_, assumptions_] := Module[{series, firstPower, values},
  ...
  If[!FreeQ[values, s23],
    Put[<|"Rows" -> values, "Expression" -> expression, "Assumptions" -> assumptions|>,
      FileNameJoin[{root, "s11_endpoint_unreduced.wl"}]];
...
results = Association@Table[
  ...
  branches = Association@Table[
    branchFile = FileNameJoin[{cache, "s11_" <> mode <> "_branch_" <> ToString[sign] <> ".wl"}];
    ...
    Put[<|"InputHash" -> inputHash, "Value" -> branchValue|>, branchFile];
...
Put[<|"Real" -> results, "Regulator" -> eps, "PhaseWithoutRecoilPower" -> phase,
  ...
  "AcceptedCacheInputHashes" -> acceptedHashes, "PriorSources" -> priorSources,
