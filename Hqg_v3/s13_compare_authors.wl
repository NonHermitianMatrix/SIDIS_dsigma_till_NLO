...
ClearAll[gate, bounded, checkFrozen, normalize, canonicalAuthors, compare,
  unitDilogarithm, reduceDilogarithm, reduceFunction, reduceAlgebraic, expandCoth];
...
reduceAlgebraic[expression_, assumptions_] := Module[{file, saved, terms, value},
...
compare[label_, left_, right_, assumptions_, allowRadicalMap_] := Module[
  {file, reducedFile, saved, expressionHash, difference, mapped, mappedAssumptions, roots, base, solutions,
    rules = {}, reduced, equality, result},
  ...
  expressionHash = Hash[{mapped, mappedAssumptions}, "SHA256"];
  reducedFile = FileNameJoin[{cache, label <> "_reduced.wl"}];
  saved = If[FileExistsQ[reducedFile], Get[reducedFile], <||>];
  reduced = If[MemberQ[acceptedInputHashes, saved["InputHash"]] && saved["ExpressionHash"] === expressionHash,
    saved["Value"],
    With[{value = bounded[normalize[mapped, mappedAssumptions], {label, "difference reduction"}]},
      Put[<|"InputHash" -> inputHash, "ExpressionHash" -> expressionHash, "Value" -> value,
        "Assumptions" -> mappedAssumptions|>, reducedFile]; value]];
  gate["comparison reduction is finite", FreeQ[reduced, Indeterminate | _DirectedInfinity]];
  If[!FreeQ[reduced, _ArcCoth],
    Print["Normalize the saved inverse-hyperbolic remainder"];
    reduced = bounded[normalize[reduced, mappedAssumptions], {label, "remaining inverse hyperbolic identity"}];
    gate["converted comparison remainder is finite", FreeQ[reduced, Indeterminate | _DirectedInfinity]];
    Put[<|"InputHash" -> inputHash, "ExpressionHash" -> expressionHash, "Value" -> reduced,
      "Assumptions" -> mappedAssumptions|>, reducedFile]];
  Print["Reduced difference leaves = ", LeafCount[reduced]];
