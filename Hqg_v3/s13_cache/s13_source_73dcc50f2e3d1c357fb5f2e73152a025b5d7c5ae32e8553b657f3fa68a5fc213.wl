...
ClearAll[gate, bounded, checkFrozen, normalize, canonicalAuthors, compare,
  unitDilogarithm, reduceDilogarithm, reduceFunction];
...
normalize[input_, assumptions_] := Module[{value, roots, rules, functions, phases, slots, formal, coefficients, result},
  ...
  coefficients = CoefficientRules[formal, slots];
  Print["Reduce ", Length[coefficients], " algebraic coefficients"];
  result = Table[
    With[{coefficient = bounded[Factor[Last[coefficients[[j]]]], {"one algebraic coefficient", j}]},
      If[Mod[j, 10] === 0, ClearSystemCache[]; Print["Reduced coefficient ", j, "/", Length[coefficients]]];
      coefficient Times @@ MapThread[Power, {functions, First[coefficients[[j]]]}]],
