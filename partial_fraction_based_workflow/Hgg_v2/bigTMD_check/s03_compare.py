...
for projection in ["Pg", "Ppp"]:
    for case in "ABC":
        name = f"NLO/{projection}/fchn4{case}.py"
...
for point in data["points"]:
    ...
    for representation, values in [("published_driver", hats), ("all_supplied_coefficients", augmented_hats)]:
...
report = {"reference_commit": manifest["commit"], "production_sha256": manifest["production_sha256"],
          ...
          "maximum_driver_mu_equals_Q_relative_difference": max(r["relative_difference"] for r in rows if r["representation"] == "published_driver" and r["parameters"]["mu2"] == r["parameters"]["Q2"]),
...
lines = ["# Hgg BigTMD comparison", "", "The supplied coefficient functions agree after their vanishing-endpoint plus pieces are included in the ordinary density. The published driver omits these pieces for Hgg and disagrees.",
         ...
         f"Maximum driver-selection relative difference at its mu=Q choice: {report['maximum_driver_mu_equals_Q_relative_difference']:.6g}.",
...
print("Published-driver discrepancy retained:", not report["all_pass"], flush=True)
