"""Compare MadGraph's explicit photon currents with the saved Hgg tensor."""
import hashlib
import itertools
import json
import math
...
gluon_average = denominator / photon_states
assert gluon_average == (benchmark["D"]-2) * (benchmark["Nc"]**2-1)
...
Path("s03_matrix.f").write_text(source.replace(
    photon_call, photon_call + "\n      CALL HGG_CURRENT(W(1,1))"))

driver = '''      PROGRAM HGG_CHECK
      IMPLICIT NONE
      INCLUDE 'coupl.inc'
...
Path("s03_result.log").write_text(run.stdout + run.stderr)
...
for index, point in enumerate(points, 1):
    values = [currents[index, mode]/coupling_norm for mode in range(6)]
...
result = {"all_pass": all(row["pass"] for row in comparisons+ward_checks),
          ...
          "incoming_gluon_average_denominator": gluon_average,
