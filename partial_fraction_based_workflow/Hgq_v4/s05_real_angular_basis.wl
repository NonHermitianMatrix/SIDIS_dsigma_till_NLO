...
gate["all real Ward identities passed", And @@
  (input["Contractions"][#] === 0 & /@ {"PhotonWard", "GluonPWard", "Gluon3Ward"})];
...
basisTask[mode_] := Catch[mode -> bounded[makeBasis[mode], mode <> " basis"], "StageFailure"];
...
configuration = KernelConfiguration["localhost", "KernelCommand" ->
  "/u/local/apps/mathematica/13.1/Executables/WolframKernel", "KernelCount" -> Min[8, slots, Length[{"Pg", "Ppp"}]],
  "TimeConstraint" -> 60];
launch = TimeConstrained[LaunchKernels[configuration], 60, $Failed];
If[!ListQ[launch], launch = TimeConstrained[LaunchKernels[Min[8, slots, Length[{"Pg", "Ppp"}]]], 60, $Failed]];
...
If[workerCount > 0,
  ...
  DistributeDefinitions[gate, bounded, atomicPut, mink, sphere, splitDenominators,
    makeBasis, basisTask, root, input, physical, angularRules];
...
values = If[workerCount > 0, ParallelMap[basisTask, {"Pg", "Ppp"}, Method -> "FinestGrained"],
  basisTask /@ {"Pg", "Ppp"}];
gate["both angular bases completed", FreeQ[values, $Failed | $Aborted]];
result = Association[values];
CloseKernels[];
atomicPut[<|"Basis" -> result, "Frame" -> frame, "AngularRules" -> angularRules,
