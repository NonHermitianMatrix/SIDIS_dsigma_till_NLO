(* Evaluate the accepted Hgq Born and real Pg/Ppp at the MadGraph points. *)
$HistoryLength = 0;
...
gate["Hgq sample and source binding", samples["Channel"] === "Hgq" &&
...
amplitudes = Get[FileNameJoin[{root, "inputs", "s01_result.wl"}]];
real = Get[FileNameJoin[{root, "inputs", "s03_result.wl"}]];
gate["accepted Born Ward identities", And @@ (# === 0 & /@ Lookup[born["BornChecks"]["Hgq"], {"PhotonWard", "GluonWard"}])];
gate["accepted real Ward identities", And @@ (# === 0 & /@
  Lookup[real["Contractions"], {"PhotonWard", "GluonPWard", "Gluon3Ward"}])];
...
Do[
  ...
  gate[kind <> " gluon initial average agrees with the generated denominator",
    (1/born["InitialAverages"]["Hgq"] /. {D -> Length[metric], SUNN -> particles["u"]["color"]}) *
      Length[particles["e-"]["helicities"]] === mg["InitialAverageDenominator"]];
  ...
  tag = Count[mg["Outgoing"], "u"]/symmetry;
  If[kind === "real", gate["S03 tag weight equals generated species weight", tag === real["TagWeight"]]];
  constants = Rationalize[mg["Couplings"], 0];
  gate[kind <> " model quark charge matches S03",
...
result = <|"Channel" -> "Hgq", "SourceSHA256" -> sha[$InputFileName], "SampleSHA256" -> sampleHash,
...
Print["HGQ_MADGRAPH_S04_SUCCESS"];
Quit[0];
