...
(* Resume unchanged masters from this channel only. *)
previousSource = FileNameJoin[{root, "s06_previous_14687645", "s06_angular_integrals.wl"}];
If[FileExistsQ[previousSource],
  gate["previous angular source identity", IntegerString[FileHash[previousSource, "SHA256"], 16, 64] === "aab95766ede4ee061eb68ad1e479f3d9afcb69549cce10c0ac8511d3d9bf869e"];
  previousHash = Hash[{FileHash[previousSource, "SHA256"],
    FileHash[FileNameJoin[{root, "s05_result.wl"}], "SHA256"]}, "SHA256"];
  legacyFiles = Select[FileNames["*.wl", cache], Get[#]["InputHash"] === previousHash &];
  gate["prior checkpoints exclude the corrected IBP branch", And @@ Table[
    With[{key = StringSplit[FileBaseName[file], "_"]},
      !(First[key] === "ML" && ToExpression[key[[2]]] > 0 && ToExpression[key[[3]]] > 1)],
    {file, legacyFiles}]];
  acceptedHashes = Append[acceptedHashes, previousHash];
  Print["Unchanged angular checkpoints eligible for reuse: ", Length[legacyFiles]]];
(* End resume bookkeeping. *)
(* Exact final factoring changes representation only. *)
previousPostprocessingSource = FileNameJoin[{root, "s06_previous_14687698", "s06_angular_integrals.wl"}];
If[FileExistsQ[previousPostprocessingSource],
  gate["previous postprocessing source identity",
    IntegerString[FileHash[previousPostprocessingSource, "SHA256"], 16, 64] === "b820fc77afb8891599d153d1944832bb283e8bb7c7accbd745fe084198a3f7e6"];
  acceptedHashes = Append[acceptedHashes, Hash[{FileHash[previousPostprocessingSource, "SHA256"],
    FileHash[FileNameJoin[{root, "s05_result.wl"}], "SHA256"]}, "SHA256"]]];
(* End final-factoring resume bookkeeping. *)
...
polynomialAngular[j_Integer, l_Integer] := Module[{polynomial, rows, integratedPhi,
  ...
  answer = Total[expansion[FunctionExpand[#], 1] & /@ Join[polarTerms, massiveTerms]];
...
mixedIBP[j_Integer, l_Integer] := Module[{a, b, flux, integrand, phiRule, xRule, rational,
  ...
  phiRule = FullSimplify[First[Solve[bb == algebraicB, yy]],
    d > 1 && -1 < c < 1 && polarDomain];
  xRule = FullSimplify[First[Solve[aa == (a /. theta -> ArcCos[x]), x]]];
  ...
  rational = Factor[FullSimplify[integrand /. phiRule,
...
evaluateMaster[key_List] := Module[{file, saved, type, j, l, value, prefactor, reduced},
  ...
  value = Map[Function[expr, Module[{factored = Collect[expr, eps, Factor]},
    gate["angular coefficient factoring reconstructs its input", Together[factored - expr] === 0];
    factored]], value];
