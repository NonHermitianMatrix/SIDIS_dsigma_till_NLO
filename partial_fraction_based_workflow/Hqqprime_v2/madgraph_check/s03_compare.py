"""Compare MadGraph's explicit photon currents with the saved Hqqprime tensor."""
import hashlib
import itertools
import json
import math
...
Path("s03_matrix.f").write_text(source.replace(
    photon_call, photon_call + "\n      CALL HQQPRIME_CURRENT(W(1,1))"))

driver = '''      PROGRAM HQQPRIME_CHECK
      IMPLICIT NONE
      INCLUDE 'coupl.inc'
...
with Path("s03_momenta.dat").open("w") as stream:
    ...
    stream.write(" ".join(format(benchmark["model_charges"][k], ".17e") for k in ["eq", "eqp"]) + "\n")
    for point in points:
        for momentum in point["momenta"]:
            stream.write(" ".join(format(x, ".17e") for x in momentum) + "\n")
        stream.write(" ".join(format(x, ".17e") for x in point["charges"]) + "\n")
...
for index, point in enumerate(points, 1):
    ...
    for name in ("g", "pp"):
        local = point[name]
        difference = abs(mg[name]-local)
        scale = max(abs(local), abs(mg[name]))
        relative = difference/scale if scale > absolute_tolerance else None
        comparisons.append({"point": index, "charges": point["charges"], "projection": name,
                            "local": local, "madgraph": mg[name],
                            "relative_difference": relative,
                            "absolute_difference": difference,
                            "pass": difference <= absolute_tolerance + relative_tolerance*scale})
...
result = {"all_pass": all(row["pass"] for row in comparisons+ward_checks),
          ...
          "zero_policy": "Near-zero contractions use the absolute tolerance; relative difference is null.",
          "ward_squared_tolerance": ward_squared_tolerance,
          "max_relative_difference": max(row["relative_difference"] or 0 for row in comparisons),
...
report = ["# MadGraph comparison", "", f"All checks pass: **{result['all_pass']}**.", "",
          ...
          f"{len(points)} kinematic-charge combinations, {len(comparisons)} contractions.", "",
...
report += [f"| {r['point']} | {r['projection']} | {r['local']:.15g} | "
           f"{r['madgraph']:.15g} | {r['relative_difference']} |" for r in comparisons]
