...
If[!FileExistsQ[FileNameJoin[{root, "s01_result.wl"}]], Print["Run s01_import_published.py first."]; Quit[1]];
...
bornPublished = (published["Born"] /. kinematics);
bornRatios = MapThread[Factor[#1/#2] &, {
  {pg, pp} /. {D -> 4, SUNN -> 3},
  {bornPublished["PgB"], bornPublished["PppB"]}
}];
gate["published Born normalization agrees in both contractions", bornRatios === {1, 1}];
...
result = <|"ColorAverage" -> color, "BornPg" -> pg, "BornPpp" -> pp,
 ...
 "BornNormalizationRatios" -> bornRatios,
