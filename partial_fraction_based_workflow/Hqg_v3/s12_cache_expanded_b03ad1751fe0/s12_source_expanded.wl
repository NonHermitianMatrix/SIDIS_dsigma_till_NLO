...
ClearAll[gate, bounded, reduce, zero, assemble];
...
assemble[mode_, sign_, distribution_] := Module[{file, saved, realPart, assumptions,
  ...
  If[FileExistsQ[file], saved = Get[file]; If[saved["InputHash"] === inputHash,
  ...
  combined = Expand[Total[inputParts]];
  powers = Range[Min[Append[Cases[Expand /@ inputParts,
    (Power[eps, n_Integer] /; n < 0) :> n, Infinity], 0]], -1];
  gate["combined input is a Laurent polynomial in the common regulator",
    PolynomialQ[Expand[eps^(-Min[Append[powers, 0]]) combined], eps]];
...
boundary = Association@Table[name -> Association@Table[distribution -> Module[{right, left, assumptions},
  ...
  right = bounded[Limit[hats[name]["NLO"][1][distribution], omega -> 0,
    Direction -> "FromAbove", Assumptions -> assumptions], {name, distribution, "right limit"}];
  left = bounded[Limit[hats[name]["NLO"][-1][distribution], omega -> 0,
    Direction -> "FromAbove", Assumptions -> assumptions], {name, distribution, "left limit"}];
...
Put[<|"StructureFunctions" -> structureFunctions, "Hats" -> hats,
  ...
  "InputHashes" -> hashes, "InputHash" -> inputHash|>, FileNameJoin[{root, "s12_result.wl"}]];
