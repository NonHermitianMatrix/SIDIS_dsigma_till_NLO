...
regularSeries[input_, assumptions_, name_, sign_] := Module[
  {expanded, terms, values, file, expressionHash, saved, value},
  ...
  gate["regular boundary partition reproduces its input",
    bounded[zero[Total[terms] - input, assumptions], "boundary partition reconstruction"]];
  values = Table[
    file = FileNameJoin[{cache, StringRiffle[{"Series", name, "Regular", ToString[sign],
      IntegerString[k, 10, 5]}, "_"] <> ".wl"}];
    expressionHash = Hash[{terms[[k]], assumptions}, "SHA256"];
    ...
    value = If[MemberQ[acceptedInputHashes, saved["InputHash"]] &&
      ...
      value = bounded[Normal[Series[terms[[k]], {omega, 0, 0}, Assumptions -> assumptions]],
      ...
      value];
    If[Mod[k, 5] === 0, ClearSystemCache[]]; value,
    {k, Length[terms]}];
  Total[values]];
...
boundarySide[name_, distribution_, sign_, assumptions_] := Module[
  ...
  value = bounded[reduce[value, interiorAssumptions], {name, distribution, sign, "boundary series reduction"}];
