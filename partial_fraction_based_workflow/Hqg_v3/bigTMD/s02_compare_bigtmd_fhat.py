"""Compare Hqg F hats with the pinned BigTMD channel-3A functions."""
import os
import sys
...
def check_inputs(data):
    require(data["Status"] == "Complete" and data["Channel"] == "Hqg", "completed Hqg export")
    require(sha(ROOT / "s01_export_fhat_benchmarks.wl") == data["SourceSHA256"], "S01 source")
    for name, record in data["IndependentInputs"]["files"].items():
        require(sha(CHANNEL / name) == record["sha256"], "frozen " + name)
    require(sha(CHANNEL / "s13_result.wl") == data["S13SHA256"], "Hqg S13 map")
    require(sha(AUTHOR / "s03_result.wl") == data["AuthorResultSHA256"], "corrected author endpoints")
    require(sha(AUTHOR / "s01_result.json") == data["AuthorMetadataSHA256"], "author metadata")
...
def main():
    data = json.loads((ROOT / "s01_result.json").read_text())
    check_inputs(data)
    ...
    for projector in ("Pg", "Ppp"):
        path = AUTHOR / "published" / (projector + "_fchn3A.py")
        record = source_records[f"NLO/{projector}/fchn3A.py"]
        require(sha(path) == record["sha256"], "Hqg channel-3A " + projector)
        source_hashes[projector] = sha(path)
        spec = importlib.util.spec_from_file_location("hqg_bigtmd_" + projector, path)
    ...
    require(set(local) == expected_interior, "complete Hqg interior inputs")
    require({(row["Benchmark"], row["Function"], row["Part"])
             for row in data["CoefficientRows"]} == expected_coefficients,
            "complete Hqg coefficient inputs")
    raw_records, direct_rows, reconstruction_rows = [], [], []
    for point in points:
        print("EVALUATE published Hqg:", point["ID"], flush=True)
    ...
    for projector in modules:
        require(sha(AUTHOR / "published" / (projector + "_fchn3A.py")) == source_hashes[projector],
    ...
    report = {"Status": "Complete", "Channel": "Hqg", "BigTMDChannel": "3A",
              "DifferenceDirection": "BigTMD minus local", "S01SHA256": sha(ROOT / "s01_result.json"),
              "S02SourceSHA256": sha(Path(__file__)), "AuthorTree": metadata["tree"],
              ...
              "Tolerance": {"Absolute": ABS_TOL, "Relative": REL_TOL}, "Benchmarks": points,
    ...
    markdown = "\n\n".join([
        ...
        "The three benchmark seeds match the named Hqqprime setup. Physics inputs are Hqg only. "
        "Settings: g_s=1, eq=1, SU(3), mu=Q, nf=4. All dependent kinematics and weights "
        "were calculated by Wolfram from the accepted Hqg maps and projectors.",
        ...
        "The author column uses the accepted corrected symbolic endpoints and rational reconstruction "
        ...
        "s01_result.json, bigtmd_fhat_benchmarks.json and bigtmd_minus_local.json.",
    ]) + "\n"
    atomic(ROOT / "bigtmd_fhat_benchmarks.json", json.dumps({"Channel": "Hqg",
           "AuthorTree": metadata["tree"], "SourceHashes": source_hashes,
           "RawProjectors": raw_records, "FHatInteriorRows": direct_rows}, indent=2) + "\n")
    atomic(ROOT / "bigtmd_minus_local.json", json.dumps(report, indent=2) + "\n")
    atomic(ROOT / "bigtmd_minus_local.md", markdown)
    print(json.dumps(summary, indent=2), flush=True)
    print("HQG_BIGTMD_S02_SUCCESS", flush=True)
