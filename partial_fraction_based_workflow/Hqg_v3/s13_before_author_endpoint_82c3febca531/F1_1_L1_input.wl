<|"InputHash" -> 
  550077100288553635958707001951328364676405219520692126344106069853849856019\
25, "Difference" -> 
  (alphaS^2*eq^2*((2*Q^2*((-16*omega^2)/(9*Pi*(omega + Q^2)) - 
         (16*omega*Q^2)/(9*Pi*(omega + Q^2)) + (16*omega*s)/
          (9*Pi*(omega + Q^2)) + (16*Q^2*s)/(9*Pi*(omega + Q^2)) - 
         2*((4*omega)/Pi - (4*s)/Pi)))/(Q^2 + s)^2 + 
      ((64*Q^2)/(9*Pi*(omega + Q^2)) + (32*omega^2)/(9*Pi*(omega + Q^2)*s) + 
        (32*Q^4)/(9*Pi*(omega + Q^2)*s) + (32*s)/(9*Pi*(omega + Q^2)) + 
        2*((16*Q^2)/(Pi*(omega + Q^2)) + (8*omega^2)/(Pi*(omega + Q^2)*s) + 
          (8*Q^4)/(Pi*(omega + Q^2)*s) + (8*s)/(Pi*(omega + Q^2))))/2))/
    Pi^2 - 16*alphaS^2*eq^2*Pi^2*
    (-1/288*(-176*Q^6*(omega - s)^5*s^5 - (omega - s)^3*s^2*
         (176*(omega - s)^5*s^3 + 528*(omega - s)^4*s^4 + 
          704*(omega - s)^3*s^5 + 352*(omega - s)^2*s^6) + 
        Q^2*(omega - s)*s*(-176*(omega - s)^6*s^4 - 704*(omega - s)^5*s^5 - 
          704*(omega - s)^4*s^6) + Q^4*(-176*(omega - s)^6*s^5 - 
          528*(omega - s)^5*s^6))/(Pi^5*(omega + Q^2)^2*(omega - s)^5*s^6) - 
     (Q^2*(176*Q^4*(omega - s)^5*s^6 + (omega - s)*s^2*
         (176*(omega - s)^6*s^4 + 352*(omega - s)^5*s^5 + 
          176*(omega - s)^4*s^6) + Q^2*s*(352*(omega - s)^6*s^5 + 
          352*(omega - s)^5*s^6)))/(144*Pi^5*(omega + Q^2)^2*(omega - s)^4*
       s^6*(Q^2 + s)^2)), "AlgebraicMap" -> {}, 
 "Assumptions" -> Q > 0 && mu > 0 && s > 0 && omega - s < 0 && 
   omega + Q^2 > 0 && (omega - s)*s < 0 && B > 0 && Element[nf, Integers] && 
   nf >= 0 && alphaS > 0 && Element[eq, Reals] && omega > 0|>
