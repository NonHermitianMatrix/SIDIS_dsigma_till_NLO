...
gate["accepted Born and virtual inputs exist", And @@
  (FileExistsQ[FileNameJoin[{root, #}]] & /@ {"s02_result.wl", "s04_result.wl", "s06_result.wl"})];
born = Get[FileNameJoin[{root, "s02_result.wl"}]];
generated = Get[FileNameJoin[{root, "s04_result.wl"}]];
virtual = Get[FileNameJoin[{root, "s06_result.wl"}]];
...
generateSelf[field_, exclusions_, label_] := Module[{topologies, diagrams, raw, amplitudes, loopFields},
  ...
  If[label === "one quark flavor",
    ...
    Put[diagrams, FileNameJoin[{root, "s09_one_flavor_diagrams.wl"}]];
  ...
  gate["generated two-point QCD coupling order", Length[amplitudes] > 0 &&
    And @@ (Exponent[#, SMP["g_s"]] == 2 && FreeQ[#, SMP["e"]] & /@ amplitudes)];
...
quarkResidue = propagatorResidue[quarkData, quark, "quark"];
gluonResidue = propagatorResidue[gluonData, gluon, "gluon and ghost"];
flavorResidue = propagatorResidue[flavorData, gluon, "one quark flavor"];
fieldResidues = <|quark -> quarkResidue["OnShell"],
...
Put[<|"Quark" -> quarkResidue, "GluonGauge" -> gluonResidue,
  ...
  FileNameJoin[{root, "s09_self_energies.wl"}]];

(* External fields belong to the same generated Compton process. *)
externalFields = {quark, gluon, quark};
...
bornGSPower = generated["CouplingDegrees"][[1, 1, 2]];
...
Put[<|"RenormalizedVirtual" -> renormalized, "CouplingResidue" -> couplingResidue,
  ...
  "CouplingWeight" -> couplingWeight, "Regulator" -> Epsilon,
  "MSPole" -> msPole, "CouplingsRemoved" -> "eq^2 gs^4",
  "InputHashes" -> Association@Table[name -> FileHash[FileNameJoin[{root, name}], "SHA256"],
    {name, {"s02_result.wl", "s04_result.wl", "s06_result.wl"}}],
  "SourceHash" -> FileHash[$InputFileName, "SHA256"]|>, FileNameJoin[{root, "s09_result.wl"}]];
Print["Wrote s09_result.wl; peak memory = ", MaxMemoryUsed[], " bytes."];
Quit[];
