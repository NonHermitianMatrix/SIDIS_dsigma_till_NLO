...
previousDirectory = FileNameJoin[{root, "s02_previous_14691692"}];
previousSource = FileNameJoin[{previousDirectory, "s02_compare_coefficients.wl"}];
previousResult = FileNameJoin[{previousDirectory, "s02_result.wl"}];
reuseEvidence = <||>;
If[FileExistsQ[previousSource] && FileExistsQ[previousResult],
  gate["completed numerical source identity", sha[previousSource] === "dfcf54bcc7416f86ec5e0b51d2dcf9a0910c5aed88f3522c38bcf3a780ff9dea"];
  gate["completed numerical snapshot identity", sha[previousResult] === "bd657e09404f83c5c1e8f5fbbd3cc3e0792c2607bbec2b2ee56bb8381d681cb4"];
  calculationBody[text_] := StringTake[text, {
    First[First[StringPosition[text, "(* Normalizations and projectors come from the imported driver expressions. *)"]]],
    First[First[StringPosition[text, "\npointTask[point_]"]]] - 1}];
  gate["numerical program is unchanged", calculationBody[Import[previousSource, "Text"]] ===
    calculationBody[Import[$InputFileName, "Text"]]];
  previous = Get[previousResult];
  previousReceipt = Import[FileNameJoin[{previousDirectory, "s02_execution.json"}], "RawJSON"];
  gate["completed numerical execution and inputs", TrueQ[previousReceipt["accepted_execution"]] &&
    previousReceipt["source_sha256"] === sha[previousSource] && previous["SourceSHA256"] === sha[previousSource] &&
    previous["FrozenInputs"] === frozen && previous["InputHash"] === Hash[{FileHash[previousSource, "SHA256"], frozen, manifest}, "SHA256"]];
  results = previous["Results"];
  gate["completed benchmarks are identical", (#["Point"] & /@ results) ===
    (KeyTake[#, {"ID", "Branch", "Input"}] & /@ points)];
  reuseEvidence = <|"SourceSHA256" -> sha[previousSource], "ResultSHA256" -> sha[previousResult],
    "JobID" -> previousReceipt["job_id"], "PeakRSSBytes" -> previousReceipt["peak_rss_bytes"]|>;
  Print["Reused complete numerical snapshot; export is serial."],
...
output = <|"Results" -> results, "FrozenInputs" -> frozen, "ReferenceCommit" -> manifest["Commit"],
  "SourceSHA256" -> sha[$InputFileName], "InputHash" -> inputHash, "NumericalSnapshot" -> reuseEvidence,
...
jsonSafe[a_Association] := Association[KeyValueMap[Function[{key, value}, key -> jsonSafe[value]], a]];
jsonSafe[a_List] := jsonSafe /@ a;
jsonSafe[a_Rational] := ToString[a, InputForm];
jsonSafe[a_] := a;
json = <|"Results" -> jsonSafe[results],
  ...
  "Flavors" -> jsonSafe[reference["Flavors"]], "NumericalSnapshot" -> reuseEvidence|>;
gate["JSON conversions evaluated", FreeQ[json, _ToString | _Rational]];
jsonFile = FileNameJoin[{root, "s02_result.json"}];
gate["JSON exported", Check[Export[jsonFile, json, "RawJSON"], $Failed] === jsonFile];
jsonRead = Import[jsonFile, "RawJSON"];
gate["JSON round trip", AssociationQ[jsonRead] && jsonRead === json];
