...
previousSource = FileNameJoin[{root, "s09_previous_14689322", "s09_real_phase_space.wl"}];
gate["eligible previous phase-space source identity", IntegerString[FileHash[previousSource, "SHA256"], 16, 64] ===
  "7d77edcee8aa0be6174e084e471ac238c70c60609fce8b0f88f73d2344cedef4"];
previousHash = Hash[Prepend[FileHash[FileNameJoin[{root, #}], "SHA256"] & /@
  {"s05_result.wl", "s06_result.wl"}, FileHash[previousSource, "SHA256"]], "SHA256"];
acceptedHashes = Append[acceptedHashes, previousHash];
regularHandlerSource = FileNameJoin[{root, "s09_previous_14689353", "s09_real_phase_space.wl"}];
gate["regular-handler checkpoint source identity", IntegerString[FileHash[regularHandlerSource, "SHA256"], 16, 64] ===
  "5f1d80b79cc35c07b960ffdd890c3b8208b3dd0bfce1e91767f0c72524f6d253"];
acceptedHashes = Append[acceptedHashes, Hash[Prepend[FileHash[FileNameJoin[{root, #}], "SHA256"] & /@
  {"s05_result.wl", "s06_result.wl"}, FileHash[regularHandlerSource, "SHA256"]], "SHA256"]];
...
(* DLMF 25.12.4: derive the inversion identity from derivative and anchor. *)
inverseSum = PolyLog[2, x] + PolyLog[2, 1/x];
inverseSlope = FullSimplify[FunctionExpand[D[inverseSum, x]], x < 0];
inversePrimitive = Integrate[inverseSlope, x, Assumptions -> x < 0, GenerateConditions -> False];
inverseConstant = FullSimplify[(inverseSum - inversePrimitive) /. x -> -1];
inverseFormula = FullSimplify[inversePrimitive + inverseConstant, x < 0];
gate["dilogarithm inversion follows from derivative and anchor",
  FreeQ[inverseFormula, _Integrate | _ConditionalExpression] &&
  FullSimplify[D[inverseSum - inverseFormula, x], x < 0] === 0 &&
  FullSimplify[(inverseSum - inverseFormula) /. x -> -1] === 0];

mixedEndpointMaster[key_, end_] := mixedEndpointMaster[key, end] = Module[
  {value = master[key]["Regular"], domain = d > 1 && -1 < c < 1,
   directions, direction, arguments, inverted, transformed, arcObjects, arcRules,
   arcForm, logObjects, logRules, functions},
  If[FreeQ[value, _PolyLog | _Log | _ArcCoth], Return[value]];
  If[TrueQ[FullSimplify[domain /. c -> end, d > 1]], direction = Automatic,
    directions = Select[{"FromAbove" -> (c > end), "FromBelow" -> (c < end)},
      FullSimplify[Last[#], -1 < c < 1] === True &];
    gate["mixed angular endpoint approach follows from its domain", Length[directions] === 1];
    direction = First[First[directions]]];
  arguments = DeleteDuplicates[Cases[value, PolyLog[2, arg_] :> arg, Infinity]];
  inverted = Select[arguments,
    Limit[#, c -> end, Direction -> direction, Assumptions -> d > 1] === -Infinity &];
  gate["inverted dilogarithm arguments are negative on the real domain",
    And @@ (FullSimplify[# < 0, domain] & /@ inverted)];
  transformed = value /. Table[PolyLog[2, arg] ->
    ((inverseFormula /. x -> arg) - PolyLog[2, Cancel[1/arg]]), {arg, inverted}];
  arcObjects = DeleteDuplicates[Cases[transformed, _ArcCoth, Infinity]];
  arcRules = Table[
    arcForm = TrigToExp[obj];
    gate["inverse hyperbolic logarithms preserve derivative and anchor",
      FreeQ[arcForm, _ArcCoth] &&
      FullSimplify[D[obj - arcForm, d], domain] === 0 &&
      FullSimplify[Limit[obj - arcForm, d -> Infinity, Assumptions -> -1 < c < 1],
        -1 < c < 1] === 0];
    obj -> arcForm, {obj, arcObjects}];
  transformed = transformed /. arcRules;
  logObjects = DeleteDuplicates[Cases[transformed, _Log, Infinity]];
  logRules = Table[obj -> PowerExpand[obj /. Log[arg_] :> Log[Factor[arg]],
    Assumptions -> domain], {obj, logObjects}];
  transformed = Expand[transformed /. logRules];
  functions = DeleteDuplicates[Cases[transformed, _Log | _PolyLog, Infinity]];
  Collect[transformed, functions, Factor]];

softRows[expression_, kappa_, assumptions_] := Module[{series, firstPower, values},
  If[FreeQ[expression, s23],
    gate["recoil-independent term has zero endpoint residue",
      bounded[Residue[expression, {s23, 0}], "regular endpoint residue"] === 0];
    Return[{}]];
  series = bounded[Series[expression, {s23, 0, -1}, Assumptions -> assumptions && s23 > 0], "soft recoil series"];
  If[series === 0, Return[{}]];
  If[!(Head[series] === SeriesData && series[[6]] === 1 && series[[5]] > -1),
  ...
  gate["soft expansion is a Laurent series", Head[series] === SeriesData && series[[6]] === 1 && series[[5]] > -1];
  gate["soft series covers the requested pole order", series[[5]] > -1];
...
softTerm[coefficient_, map_, powers_, sign_] := Module[{assumptions, replacement, prefactor,
  ...
  If[map["Type"] === "ML",
    ...
    end = FullSimplify[Limit[c /. rules, s23 -> 0, Direction -> "FromAbove",
      Assumptions -> assumptions], assumptions];
    Return[softRows[phase prefactor (mixedEndpointMaster[{"ML", j, l}, end] /. rules),
...
If[workerCount > 0,
  ...
  DistributeDefinitions[gate, bounded, atomicPut, es, master, geometry, reduce,
    ...
    mixedEndpointMaster, inverseFormula,
...
results = Association@Table[
  ...
  branches = Association@Table[
    ...
    combined = Table[Print[mode, " branch ", sign, " soft group ", Take[group[[1]], 2]];
      ...
      {group, groups}];
    Do[If[row[[2]] < -1,
      recoilResidual = bounded[FullSimplify[row[[3]], assumptions], "stronger recoil pole"];
      atomicPut[<|"Tensor" -> mode, "Branch" -> sign, "Power" -> Take[row, 2],
        "Residual" -> recoilResidual, "Assumptions" -> assumptions, "InputRows" -> rows,
        "CombinedRows" -> combined, "InputHash" -> inputHash|>,
        FileNameJoin[{root, "s09_recoil_" <> mode <> "_" <> ToString[sign] <> ".wl"}]];
      Print["Stronger-recoil residual ", mode, " ", sign, " ", Take[row, 2], ": ", InputForm[recoilResidual]];
      gate["stronger recoil poles cancel", recoilResidual === 0]], {row, combined}];
    ...
    sign -> branchValue],
    {sign, {1, -1}}];
  mode -> <|"Ordinary" -> ordinary, "Branches" -> branches|>, {mode, input["TensorKeys"]}];
CloseKernels[];
atomicPut[<|"Real" -> results, "Components" -> input["Components"],
  "OtherChargeMomentDefinitions" -> input["OtherChargeMomentDefinitions"],
  "TensorKeys" -> input["TensorKeys"], "ParallelRuntime" -> runtimeEvidence, "Regulator" -> eps, "PhaseWithoutRecoilPower" -> phase,
