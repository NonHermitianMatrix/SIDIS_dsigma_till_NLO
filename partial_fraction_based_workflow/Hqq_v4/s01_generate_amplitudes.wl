(* Fresh Hqq amplitudes for paper Table I and the Born/vertex inputs for Eq. 46. *)
$HistoryLength = 0;
...
quarkField = F[3, {1}];
otherQuarkField = F[3, {2}];
massRules = Thread[(SMP /@ {"m_u", "m_d", "m_s", "m_c", "m_b", "m_t"}) -> 0];
excluded = {S[_], V[1 | 2 | 3 | 4], U[1 | 2 | 3 | 4], F[1 | 2]};
requests = <|
 "Born" -> <|"Loops" -> 0, "Incoming" -> {V[1], quarkField}, "Outgoing" -> {quarkField, V[5]},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2}|>,
 "RealQGG" -> <|"Loops" -> 0, "Incoming" -> {V[1], quarkField}, "Outgoing" -> {quarkField, V[5], V[5]},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2, k3}|>,
 "RealSame" -> <|"Loops" -> 0, "Incoming" -> {V[1], quarkField}, "Outgoing" -> {quarkField, quarkField, -quarkField},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2, k3}|>,
 "RealDistinct" -> <|"Loops" -> 0, "Incoming" -> {V[1], quarkField}, "Outgoing" -> {quarkField, otherQuarkField, -otherQuarkField},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2, k3}|>,
 "Virtual" -> <|"Loops" -> 1, "Incoming" -> {V[1], quarkField}, "Outgoing" -> {quarkField, V[5]},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2}|>,
 "AuxHgqBorn" -> <|"Loops" -> 0, "Incoming" -> {V[1], V[5]}, "Outgoing" -> {quarkField, -quarkField},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2}|>,
 "AuxHqgBorn" -> <|"Loops" -> 0, "Incoming" -> {V[1], quarkField}, "Outgoing" -> {V[5], quarkField},
   "IncomingMomenta" -> {q, p}, "OutgoingMomenta" -> {k1, k2}|>,
 "ChargeVertex" -> <|"Loops" -> 0, "Incoming" -> {V[1]}, "Outgoing" -> {quarkField, -quarkField},
   "IncomingMomenta" -> {q}, "OutgoingMomenta" -> {k1, k2}|>,
 "OtherChargeVertex" -> <|"Loops" -> 0, "Incoming" -> {V[1]}, "Outgoing" -> {otherQuarkField, -otherQuarkField},
   "IncomingMomenta" -> {q}, "OutgoingMomenta" -> {k1, k2}|>|>;
species[field_] := Which[field === V[1], photon, field === V[5], gluon,
  field === quarkField, quark, field === -quarkField, antiquark,
  field === otherQuarkField, otherQuark, field === -otherQuarkField, otherAntiquark,
  True, Missing["UnknownField", field]];
...
result = Join[amplitudes, <|"GraphCounts" -> Map[#["GraphCount"] &, packets],
  ...
  "RealSectors" -> Select[Keys[requests], StringStartsQ[#, "Real"] &],
  "BornInputs" -> Select[Keys[requests], # === "Born" || StringEndsQ[#, "Born"] &],
