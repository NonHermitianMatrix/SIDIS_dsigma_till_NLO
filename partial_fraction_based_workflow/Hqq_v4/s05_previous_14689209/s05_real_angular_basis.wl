...
verifyBasis[mode_, value_] := Module[{denominators, reconstructed},
  If[TrueQ[Lookup[value, "ExchangeOdd", False]],
    Return[bounded[Factor[input["Contractions"][mode] +
      (input["Contractions"][mode] /. exchangeRules)], "cached spectator parity"] === 0]];
  ...
  bounded[Together[input["Contractions"][mode] - reconstructed], "cached angular reconstruction"] === 0];
...
If[workerCount > 0,
  ...
  DistributeDefinitions[gate, bounded, atomicPut, mink, sphere, splitDenominators,
    makeBasis, verifyBasis, basisTask, root, input, physical, angularRules, exchangeRules,
    basisCache, basisHash, previousCache, previousHash];
