...
functionRecords={};
heldC[HoldComplete[xx_]] := ToString[Unevaluated[xx],CForm];
...
prepare[e0_,branch_,label_] := Module[{e=e0,rr,atoms,mplRules,h,zeroCoefficients,zeroDegree},
 ...
 If[MemberQ[{"Hgq_v3","Hqq_v2"},channel],
   ...
   rr={Sqrt[(s+t)^2]->Refine[Sqrt[(s+t)^2],If[branch===1,s+t>0,s+t<0]],
       Sqrt[s^2+2 s t+t^2]->Refine[Sqrt[s^2+2 s t+t^2],If[branch===1,s+t>0,s+t<0]]};
   e=e/.rr;
 ...
 mplRules=Table[
   h=HyperIntica`MplAsHlog@@(List@@a);
...
exportFunction[e0_,label_,branch_,endpoint_,jacobianIncluded_,chargeCase_:"A"] := Module[
 ...
 assignments=Cases[held,HoldPattern[Set[l_Symbol,r_]]:>{ToString[l,InputForm],ToString[r,CForm]},Infinity];
...
Switch[channel,
 ...
 Do[ee=data["FiniteHattedStructureFunctions",hat];
  ...
  ee=First[ee]/.S13ConvolutionTest[_]->1;
