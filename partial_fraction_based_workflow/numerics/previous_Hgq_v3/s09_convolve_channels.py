...
def derive_and_build():
    ...
    measure_contract = load('s10_cache/s10_consumer_contract_result')
    assert measure_contract['status'] == 'Complete' and all(measure_contract['checks'].values())
    assert all(not b['JacobianAlreadyIncluded'] for b in kernels['bundles'] if b['Channel'] == 'Hqq_v2')
    ...
    contract = dict(stage='s09', status='IntegrandBuilt', producer_sha256=sha(Path(__file__)),
        input_hashes={name: sha(ROOT/name) for name in ['s02_result', 's04_result', 's06_result', 's07_result', 's08_result', 's10_result', 's10_cache/s10_consumer_contract_result', 'dsigmapibydpt/s01_result']},
...
def integrate_bin(task):
    spec, neval, nitn, seed, build_hash, refinements = task
    destination = CACHE/(spec['id']+'_result')
    if destination.exists():
        old = json.loads(destination.read_text())
        if old['build_sha256'] == build_hash and old.get('initial_neval') == neval and old['nitn'] == nitn and old['status'] == 'Complete':
    ...
    for level in range(refinements+1):
        ...
        result = vegas.ravg(raw.itn_results, weighted=False)
        means = np.asarray(gvar.mean(result)); cov = np.asarray(gvar.evalcov(result))
        iterations = np.array([np.asarray(gvar.mean(r))[1:3] for r in raw.itn_results])
        variances = np.array([np.diag(np.asarray(gvar.evalcov(r)))[1:3] for r in raw.itn_results])
...
def main():
    ...
    tasks = [(b, options.neval, options.nitn, 730001+bins().index(b), sha(CACHE/'s09_build_result'), options.refinements) for b in selected]
    results = []
