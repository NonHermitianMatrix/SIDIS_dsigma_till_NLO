...
def record(message):
    with (ROOT.parent/'progress.md').open('a') as f:
        f.write('\nNumerics S10 '+datetime.now(timezone.utc).isoformat()+': '+message+'\n')
...
'''.replace('INPUT',json.dumps(str(ROOT.parent/'Hqq_v2/s04_result.wl'))).replace('OUTPUT',json.dumps(str(out))).replace('JACOBIAN',Jwl)
    ...
    run=subprocess.run(['/home/physics/wolframengine/opt/Wolfram/WolframEngine/15.0/Executables/WolframKernel','-noinit','-noprompt','-script',str(path)],capture_output=True,text=True,timeout=60)
    ...
    sources=[ROOT.parent/'Hqq_v2'/s for s in ['s04_integrate_hqq_phase_space.wl','s05_expand_hqq_endpoints.wl',
    ...
    result=dict(stage='s10_consumer_contract',status='Complete',producer_sha256=sha(Path(__file__)),
        channel='Hqq_v2',jacobian_already_included=jacobian_included,checks=checks,
...
def invariant_contract():
    ...
    samples=json.loads((ROOT/'s10_cache/s10_pilot_scan').read_text())[:2]
    samples += [r for r in json.loads((ROOT/'s09_cache/s09_smoke_result').read_text())['points'] if r['diagnostics'][0]>0]
    benchmark_rows=[]
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--dps',nargs='+',type=int,default=[100,140]);parser.add_argument('--scan-pilot',action='store_true');parser.add_argument('--use-pilot',action='store_true');parser.add_argument('--consumer-contract',action='store_true');parser.add_argument('--invariant-contract',action='store_true');parser.add_argument('--channels',nargs='+',default=['Hqq_v2','Hgq_v3']);parser.add_argument('--kernels-result',default='s07_result');options=parser.parse_args()
    ...
    rows=json.loads((ROOT/'s09_cache/s09_smoke_result').read_text())['points']
    if options.use_pilot:rows=json.loads((ROOT/'s10_cache/s10_pilot_scan').read_text())[:2]
    checks=[]
    for row_index,row in enumerate(rows):
        if row['diagnostics'][0]<=0:continue
        _,xi,zeta0,zeta,Q2,y,pt,zH,ss,B=row['diagnostics']
        for channel,label in [(c,l) for c in options.channels for l in ['Delta','Regular','L0']]:
            ...
            arguments=np.array([Q2,values['s'],values['t'],sub['s23'],B,values['mu2'],values['xHat'],zH,pt**2,sub['xB'],xi,
                pdf.sidis_alpha(values['mu2']),float(S.sympify(group['charge'])),float(S.sympify(group['other_charge'])),
                group['same_charge_flavor_count'],group['other_charge_flavor_count'],nf,1,abs(values['s']+values['t'])],dtype=float)
    ...
    result=dict(stage='s10',status='Complete' if checks and all(c['passed'] for c in checks) else 'ValidationFailed',
        producer_sha256=sha(Path(__file__)),s07_result_sha256=sha(kernel_path),kernel_receipt=options.kernels_result,checks=checks)
