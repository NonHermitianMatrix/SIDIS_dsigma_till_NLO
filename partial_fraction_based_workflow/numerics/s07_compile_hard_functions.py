...
def main():
    ...
    parser.add_argument('--channels',nargs='+',default=['Hqq_v4','Hgg_v2','Hqqbar_v2','Hqqprime_v2','Hgq_v4','Hqg_v3'])
    ...
    archived=json.loads((ROOT/'previous_six_input_replacement/s01_result').read_text())
    payloads={row['channel']:next(f['sha256'] for f in row['files'] if f['role']=='terminal_fhat_payload') for row in manifest['channels']}
    old_payloads={row['channel']:next(f['sha256'] for f in row['files'] if f['role']=='terminal_fhat_payload') for row in archived['channels']}
    for spec in specs:
        channel=spec['Channel']
        if 'FhatInputSHA256' in spec:assert spec['FhatInputSHA256']==payloads[channel]
        else:
            assert channel in ['Hgq_v4','Hqg_v3'] and payloads[channel]==old_payloads[channel]
            accepted=json.loads((ROOT/'previous_six_input_replacement/s05_cache'/(channel+'_result')).read_text())
            assert spec in accepted['Functions']
    ...
    if len(options.channels)==6:
        overview=dict(stage='s05',status='Complete',channels=options.channels,s01_result_sha256=sha(ROOT/'s01_result'),
    ...
    result=dict(stage='s07',status='Complete' if all(c['passed'] for c in checks) else 'ValidationFailed',producer_sha256=sha(Path(__file__)),
        channels=options.channels,s01_result_sha256=sha(ROOT/'s01_result'),library=str(libpath.relative_to(ROOT)),library_sha256=sha(libpath),functions=functions,bundles=bundles,checks=checks,
