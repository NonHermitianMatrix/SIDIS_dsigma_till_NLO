...
def main():
    ...
    contract_path=ROOT/'s03_cache/Hgq_v4_consumer_contract'
    contract=json.loads(contract_path.read_text())
    assert contract['Status']=='Complete' and all(contract['Checks'].values())
    assert contract['InputSHA256']==sha(ROOT/'Fhats/Hgq_v4/s10_result.wl')
    charge_degree=contract['ChargeDegree']
    ...
    for active_names in [['u','d','s','c'],['u','d','s','c','b']]:
        ...
        lum=dict(Hgg=sum(charge[i]**2 for i in quarks)*f[0]*d[0],
            Hgq_v4=f[0]*sum(charge[i]**charge_degree*d[i] for i in active),
    ...
    actions={}
    plus_arguments={'Hqq_v2':ss,'Hqg_v3':ss/B,
                    'Hgq_v4':S.sympify(contract['PlusLogArgument'],locals={'ss':ss,'B':B})}
    ...
    previous=json.loads((ROOT/'previous_Hgq_v3/s08_result').read_text())
    for nf,old in previous['luminosities'].items():
        current=luminosities[nf]
        checks['unchanged_flavor_actions_nf'+nf]=all(S.expand(S.sympify(v)-S.sympify(current['expressions'][k]))==0
            for k,v in old['expressions'].items() if k!='Hgq_v3') and old['hqq_charge_groups']==current['hqq_charge_groups']
    assert all(checks.values()),checks
    result=dict(stage='s08',status='Complete',producer_sha256=sha(Path(__file__)),
        inputs={p.name:sha(p) for p in [ROOT/'s04_result',ROOT/'s06_result',model,contract_path]},
