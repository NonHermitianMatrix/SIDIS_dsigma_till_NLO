...
If[mode === "inspect",
 ...
 Print["S05_INSPECTION_DONE"];finish[0]
];
...
boundaryRows={};
If[channel==="Hgq_v4",
 v4Contract=Import[FileNameJoin[{root,"s03_cache","Hgq_v4_consumer_contract"}],"RawJSON"];
 ...
 require[v4Contract["InputSHA256"]===FileHash[FileNameJoin[{root,"Fhats",channel,"s10_result.wl"}],"SHA256","HexString"],"v4 contract input identity"];
...
prepare[e0_,branch_,label_] := Module[{e=e0,rr,atoms,mplRules,h,zeroCoefficients,zeroDegree,original},
 ...
 original=e;
 If[StringContainsQ[label,"Regular"] || channel==="Hgq_v4",
...
exportFunction[e0_,label_,branch_,endpoint_,jacobianIncluded_,chargeCase_:"A"] := Module[
 ...
 ret=<|"Name"->name,"Channel"->channel,"Label"->label,"Branch"->branch,"Endpoint"->endpoint,
   ...
   "CFormExactRationals"->True,"RationalCoefficientReconstruction"->(!StringContainsQ[label,"Regular"] && channel=!="Hgq_v4"),
   "PreparationMethod"->If[StringContainsQ[label,"Regular"] || channel==="Hgq_v4","ExactUncollected","ExactRationalCoefficientCollection"],
...
Switch[channel,
 ...
 "Hgq_v4",
 data=Get[FileNameJoin[{root,"Fhats",channel,"s10_result.wl"}]];
 (* The v4 S10 producer normalizes partonic contractions. Its saved hats
    have no hadronic-variable Jacobian; the accepted S04 consumer supplies it. *)
 require[FreeQ[data["Hats"],zH|PHT2|xi|xB|zeta],"v4 partonic coefficient variables"];
 hats=If[StringQ[selectedHat],{selectedHat},Keys[data["Hats"]]];
 branches=If[StringQ[selectedBranch],{ToExpression[selectedBranch]},Append[Keys[data["BranchCoordinates"]],0]];
 require[SubsetQ[Keys[data["Hats"]],hats] && SubsetQ[Append[Keys[data["BranchCoordinates"]],0],branches],"v4 work selectors"];
 Do[
  exportFunction[data["Hats"][hat]["LODelta"],hat<>"_LODelta",branch,True,False];
