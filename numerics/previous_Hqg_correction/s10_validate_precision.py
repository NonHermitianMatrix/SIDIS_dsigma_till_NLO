...
'''.replace('INPUT',json.dumps(str(ROOT.parent/'Hqq_v2/s04_result.wl'))).replace('OUTPUT',json.dumps(str(out))).replace('JACOBIAN',Jwl)
    ...
    sources=[ROOT.parent/'Hqq_v4'/s for s in ['s04_integrate_hqq_phase_space.wl','s05_expand_hqq_endpoints.wl',
    ...
    result=dict(stage='s10_consumer_contract',status='Complete',producer_sha256=sha(Path(__file__)),
        channel='Hqq_v4',jacobian_already_included=jacobian_included,checks=checks,
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--dps',nargs='+',type=int,default=[100,140]);parser.add_argument('--scan-pilot',action='store_true');parser.add_argument('--use-pilot',action='store_true');parser.add_argument('--consumer-contract',action='store_true');parser.add_argument('--invariant-contract',action='store_true');parser.add_argument('--channels',nargs='+',default=['Hqq_v4','Hgg_v2','Hqqbar_v2','Hqqprime_v2','Hgq_v4','Hqg_v3']);parser.add_argument('--kernels-result',default='s07_result');parser.add_argument('--sample-source');options=parser.parse_args()
