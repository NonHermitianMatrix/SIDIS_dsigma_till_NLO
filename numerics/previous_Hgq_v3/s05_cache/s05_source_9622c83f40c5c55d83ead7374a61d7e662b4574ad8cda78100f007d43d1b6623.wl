...
evaluatePrepared[expression_,substitutions_,label_] := Module[{value,degree,coefficients},
 value=(expression/.Log[a_]:>logHold[a])/.substitutions;
 ...
 value=N[value,100];
 require[NumberQ[value] && FreeQ[value,Indeterminate|ComplexInfinity|DirectedInfinity[_]],"nonfinite Wolfram sample "<>label];value
];
...
exportFunction[e0_,label_,branch_,endpoint_,jacobianIncluded_,chargeCase_:"A"] := Module[
 ...
 rows=Select[Join[maps["benchmark_rows"],invariantContract["benchmark_rows"]],#["branch"]===branch && #["endpoint"]===endpoint&];
