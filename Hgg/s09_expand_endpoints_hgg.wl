...
assert[s08["Channel"] === "Hgg only", "s08_result is not Hgg-only."];
...
assert[
  ...
  "The S08 source binding is stale."
];
...
assert[AssociationQ[realKernels],
  "The sole Hgg real kernel did not load as an Association."];
assert[Sort[Keys[realKernels]] === Sort[{"Pg", "PPP"}],
  "The Hgg real kernel does not contain exactly Pg and PPP."];
...
validateExpandedPair[pair_Association] := Module[{},
  assert[Sort[Keys[pair]] === Sort[{"Pg", "PPP"}],
    "The expanded Hgg pair has the wrong projector keys."];
  assert[AllTrue[Values[pair], FreeQ[#, _S08Case2Master] &],
    "An expanded Hgg projector retains S08Case2Master."];
  assert[AllTrue[Values[pair], FreeQ[#, _Hypergeometric2F1 | _Beta] &],
    "An expanded Hgg projector retains a B18 special function."];
  True
];
...
loadOrBuildExpansion[projector_String] := Module[
  ...
  Print["S09_STAGE: Appendix-F expansion for Hgg;q_qbar " <> projector];
...
Print["S09_STAGE: expanding Hgg real angular masters serially"];
...
assert[AllTrue[Values[weightedReal], ! FreeQ[#, S09HggFlavorChargeSum] &],
  "The physical Hgg flavor-charge factor was not applied."];
...
Print["S09_STAGE: constructing Hgg s23 endpoint distributions"];
endpointData = AssociationMap[
  endpointDistributionData[weightedReal[#], s23UpperB, #] &,
  {"Pg", "PPP"}
];
...
validateEndpointPair[pair_Association] := Module[{},
  ...
  assert[AllTrue[Values[pair], ! FreeQ[#, DiracDelta[s23]] &],
    "An endpoint-expanded Hgg projector lacks DiracDelta[s23]."];
  assert[AllTrue[Values[pair], ! FreeQ[#, _S09PlusDistribution] &],
    "An endpoint-expanded Hgg projector lacks plus distributions."];
  True
];
...
s09Result = <|
  ...
  "Checks" -> s09Checks,
  "MemoryStrategy" ->
    "Pg and PPP expanded serially into atomic source-bound caches; s09_result stores cache references rather than duplicate expanded kernels",
