module hgg_madgraph_c_bridge
  use, intrinsic :: iso_c_binding
  implicit none

contains

  subroutine hgg_mg_init(c_path) bind(C, name="hgg_mg_init")
    ...
    if (.not. found_terminator) then
      error stop "HGG_MG_FATAL: parameter-card path exceeds 511 bytes"
    end if

    call setparamlog(.false.)
    call setpara(fortran_path)
  end subroutine hgg_mg_init

  subroutine hgg_mg_eval(momentum, alpha_s, answer) &
      bind(C, name="hgg_mg_eval")
    ...
    pdgs = [11_c_int, 21_c_int, 11_c_int, 21_c_int, 2_c_int, -2_c_int]
    process_id = 1_c_int
  ...
  end subroutine hgg_mg_eval

  subroutine hgg_mg_metadata(pdgs, identity_factor) &
      bind(C, name="hgg_mg_metadata")
    integer(c_int), intent(out) :: pdgs(6)
    integer(c_int), intent(out) :: identity_factor

    pdgs = [11_c_int, 21_c_int, 11_c_int, 21_c_int, 2_c_int, -2_c_int]
    identity_factor = 32_c_int
  end subroutine hgg_mg_metadata

end module hgg_madgraph_c_bridge
