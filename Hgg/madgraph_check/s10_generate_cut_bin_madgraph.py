...
BRIDGE = ROOT / "s03_libhgg_madgraph_bridge.so"
...
EXPECTED_SHA256 = {
    BRIDGE: "c9fb2cf14e8c404c177c859283c8ea794a67cd86f4df9d75142be386ee759fa3",
    PARAM_CARD: "55bb009a781370ab3fdfb523be00d0c8f68ffec442d12eb41b8d2da50307cf64",
    POINT_VALIDATION: "59ac11e6055141c0edc521def89a24c4743dae6e8f3ebb043400e673f4b56fdb",
    LOCAL_EVALUATOR: "8d34213d140d7bd8d3df3f5594a351ad864d3bceb937a3a92243b3d7ee28e32c",
}
...
def four_angle_average(
    ...
    for index in range(4):
        ...
        values.append(identity_correction * evaluate(library, rotated, alpha_s))
...
def main() -> None:
    ...
    print(
        "S10_STAGE: generating deterministic massless four-body RAMBO trials",
        flush=True,
    )
    ...
    incoming_gluon = np.array([SQRT_S / 2.0, 0.0, 0.0, -SQRT_S / 2.0])
    ...
    outgoing_k1_gluon = final_momenta[:, 1]
    outgoing_k2_up = final_momenta[:, 2]
    outgoing_k3_antiup = final_momenta[:, 3]
    photon = incoming_electron[None, :] - outgoing_electron

    q2 = -minkowski_dot(photon, photon)
    s12 = minkowski_dot(
        outgoing_k1_gluon + outgoing_k2_up,
        outgoing_k1_gluon + outgoing_k2_up,
    )
    s13 = minkowski_dot(
        outgoing_k1_gluon + outgoing_k3_antiup,
        outgoing_k1_gluon + outgoing_k3_antiup,
    )
    s23 = minkowski_dot(
        outgoing_k2_up + outgoing_k3_antiup,
        outgoing_k2_up + outgoing_k3_antiup,
    )
    shat = s12 + s13 + s23
    u1 = minkowski_dot(
        incoming_gluon[None, :] - outgoing_k1_gluon,
        incoming_gluon[None, :] - outgoing_k1_gluon,
    )
    u2 = minkowski_dot(
        incoming_gluon[None, :] - outgoing_k2_up,
        incoming_gluon[None, :] - outgoing_k2_up,
    )
    u3 = minkowski_dot(
        incoming_gluon[None, :] - outgoing_k3_antiup,
        incoming_gluon[None, :] - outgoing_k3_antiup,
    )
    t1 = minkowski_dot(photon - outgoing_k1_gluon, photon - outgoing_k1_gluon)
    t2 = minkowski_dot(photon - outgoing_k2_up, photon - outgoing_k2_up)
    t3 = minkowski_dot(photon - outgoing_k3_antiup, photon - outgoing_k3_antiup)
    ...
    for accepted_position, trial_index in enumerate(selected_indices):
        lab_momenta = np.vstack(
            [incoming_electron, incoming_gluon, final_momenta[trial_index]]
        )
        ...
        if accepted_position < 8:
            ...
            spot_hadronic_total = incoming_gluon + spot_photon
    ...
    metadata = {
        "stage": "HggFiniteCutMadGraphSample-v1",
        "status": "complete",
        "sqrt_s_gev": SQRT_S,
        "s_gev2": S,
        ...
        "local_and_generated_final_order": ["g(k1)", "u(k2)", "ubar(k3)"],
        "cuts": CUTS,
