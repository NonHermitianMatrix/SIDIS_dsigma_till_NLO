program hgg_direct_madgraph_reference
  ...
  if (len_trim(parameter_card) == 0) then
    error stop "HGG_MG_FATAL: parameter-card argument is required"
  end if
  ...
  write(*, '(A,ES26.17E3)') "HGG_DIRECT_MATRIX=", matrix_element
  write(*, '(A,ES26.17E3)') "HGG_DIRECT_MAX_CONSERVATION=", &
    maxval(abs(momentum_residual))
  write(*, '(A,ES26.17E3)') "HGG_DIRECT_MAX_MASS2=", &
    maxval(abs(mass_squared))
end program hgg_direct_madgraph_reference
