...
(*
  ...
  s05_result.  In D dimensions this stage performs, in order,
    ...
    1. physical axial polarization sums for incoming-gluon p and
       fragmenting-gluon k1, using the lightlike spectator k2 as reference,
...
stageVersion = "HggS06-v2";
preflightOnly = Environment["HGG_S06_PREFLIGHT_ONLY"] === "1";
...
setThreeBodyKinematics[] := (
  ...
  FeynCalc`SP[k2, k2] = 0;
  FeynCalc`SP[k3, k3] = 0;
  FeynCalc`SP[p, q] = (sHat + Q2)/2;
  FeynCalc`SP[q, k1] = (-Q2 - t1)/2;
  FeynCalc`SP[q, k2] = (-Q2 - t2)/2;
  FeynCalc`SP[q, k3] = (-Q2 - t3)/2;
  FeynCalc`SP[p, k1] = -u1/2;
  FeynCalc`SP[p, k2] = -u2/2;
  FeynCalc`SP[p, k3] = -u3/2;
  FeynCalc`SP[k1, k2] = s12/2;
  FeynCalc`SP[k1, k3] = s13/2;
  FeynCalc`SP[k2, k3] = s23/2;
);

polarizationSumMomenta = {p, k1};
polarizationReferenceMomentum = k2;
polarizationSumSpecifications =
  ({#, polarizationReferenceMomentum} & /@ polarizationSumMomenta);

Print["S06_STAGE: gating physical polarization references"];
setThreeBodyKinematics[];
polarizationReferenceDotProducts = Factor[
  FeynCalc`FCI[FeynCalc`SPD[First[#], Last[#]]]
] & /@ polarizationSumSpecifications;
polarizationReferenceNorms = Factor[
  FeynCalc`FCI[FeynCalc`SPD[Last[#], Last[#]]]
] & /@ polarizationSumSpecifications;
physicalPolarizationReferenceChecks = MapThread[
  ! TrueQ[#1 === 0] && TrueQ[#2 === 0] &,
  {polarizationReferenceDotProducts, polarizationReferenceNorms}
];
assert[
  And @@ physicalPolarizationReferenceChecks,
  "A physical polarization reference is null-collinear or non-lightlike."
];
FeynCalc`FCClearScalarProducts[];

Print["S06_STAGE: deriving the incoming-gluon averaging divisor"];
FeynCalc`SPD[s06CountP, s06CountP] = 0;
FeynCalc`SPD[s06CountN, s06CountN] = 0;
polarizationCountPair = FeynCalc`Pair[
  FeynCalc`Momentum[FeynCalc`Polarization[s06CountP, I], D],
  FeynCalc`Momentum[FeynCalc`Polarization[s06CountP, -I], D]
];
polarizationMetricTrace = FeynCalc`Contract[
  FeynCalc`DoPolarizationSums[
    polarizationCountPair,
    s06CountP,
    s06CountN,
    FeynCalc`FCVerbose -> 0
  ]
];
incomingPhysicalPolarizationStates = Factor[-polarizationMetricTrace];
incomingAdjointColorStates = FeynCalc`SUNN^2 - 1;
initialGluonAverageDenominator = Factor[
  incomingPhysicalPolarizationStates incomingAdjointColorStates
];
assert[
  TrueQ[
    Factor[incomingPhysicalPolarizationStates - (D - 2)] === 0
  ] &&
    TrueQ[
      Factor[
  ...
  "The incoming-gluon averaging divisor was not derived correctly."
];
FeynCalc`FCClearScalarProducts[];

If[preflightOnly,
  Print[
    "S06_PREFLIGHT_REFERENCE_SPECIFICATIONS=",
    InputForm[polarizationSumSpecifications]
  ];
  Print[
    "S06_PREFLIGHT_REFERENCE_DOT_PRODUCTS=",
    InputForm[polarizationReferenceDotProducts]
  ];
  Print[
    "S06_PREFLIGHT_REFERENCE_NORMS=",
    InputForm[polarizationReferenceNorms]
  ];
  Print[
    "S06_PREFLIGHT_REFERENCE_CHECKS=",
    InputForm[physicalPolarizationReferenceChecks]
  ];
  Print[
    "S06_PREFLIGHT_INCOMING_AVERAGE=",
    InputForm[initialGluonAverageDenominator]
  ];
  Print["S06_PREFLIGHT_SUCCESS"];
  Quit[0]
];

assert[
  ! FileExistsQ[resultPath],
  "s06_result already exists; refusing to overwrite it."
];
...
If[MissingQ[finalTensor],
  ...
  If[MissingQ[postDiracTensor],
    ...
    Print[
      "S06_STAGE: summing physical p and k1 gluon polarizations"
    ];
...
s06Checks = <|
  ...
  "PhysicalAxialPolarizationSumsUsed" -> True,
  "PhysicalPolarizationReferenceLightlike" ->
    And @@ (TrueQ[# === 0] & /@ polarizationReferenceNorms),
  "PhysicalPolarizationReferenceNonCollinear" ->
    And @@ (! TrueQ[# === 0] & /@ polarizationReferenceDotProducts),
...
s06Result = <|
  ...
  "InitialStateAverage" ->
    1/initialGluonAverageDenominator,
  "InitialSpinStates" -> incomingPhysicalPolarizationStates,
  "InitialColorStates" -> incomingAdjointColorStates,
  "PolarizationSumConvention" ->
    "FeynCalc physical axial sums for incoming p and fragmenting k1, using the lightlike spectator k2 as reference.",
  "PolarizationSumLedger" -> <|
    "Momenta" -> polarizationSumMomenta,
    "ReferenceMomentum" -> polarizationReferenceMomentum,
    "Specifications" -> polarizationSumSpecifications,
    "ReferenceDotProducts" -> polarizationReferenceDotProducts,
    "ReferenceNorms" -> polarizationReferenceNorms,
    "PhysicalReferenceChecks" -> physicalPolarizationReferenceChecks,
    "PolarizationMetricTrace" -> polarizationMetricTrace
  |>,
