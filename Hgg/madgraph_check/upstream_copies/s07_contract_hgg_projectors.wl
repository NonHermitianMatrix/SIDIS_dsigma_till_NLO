...
assert[
  s06["CacheProvenance", "StageVersion"] === "HggS06-v2",
  "The S06 input does not have physical-projector v2 provenance."
];
assert[
  s06["Checks", "PhysicalAxialPolarizationSumsUsed"] === True &&
    And @@ (
      TrueQ /@ s06[
        "PolarizationSumLedger", "PhysicalReferenceChecks"
      ]
    ) &&
    And @@ (
      TrueQ[# === 0] & /@
        s06["PolarizationSumLedger", "ReferenceNorms"]
    ) &&
    And @@ (
      ! TrueQ[# === 0] & /@
        s06["PolarizationSumLedger", "ReferenceDotProducts"]
    ),
  "The S06 physical-polarization reference ledger is not accepted."
];
assert[
...
s07Checks = <|
  ...
  "S06PhysicalProjectorProvenanceVerified" -> True,
