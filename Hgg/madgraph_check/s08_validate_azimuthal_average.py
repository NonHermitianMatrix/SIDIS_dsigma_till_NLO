...
EXPECTED_SHA256 = {
    ...
    LOCAL_POINT: "65eaaea96974c808c0a3008e8da97a708b26dd22bbeb4026f6a9467316165871",
}
...
def boost_to_rest(momentum: list[float], beta: list[float]) -> list[float]:
    ...
    spatial = [
        momentum[index + 1] + coefficient * beta[index]
...
def evaluate(
    ...
    flat = [component for momentum in momenta for component in momentum]
    momentum_array = (ctypes.c_double * 24)(*flat)
...
def main() -> None:
    ...
    if (
        local_point.get("stage") != "HggLocalTensorAndProjectionValidation-v1"
    ...
    incoming_gluon_spatial = rest_momenta[1][1:]
    gluon_norm = math.sqrt(sum(component * component for component in incoming_gluon_spatial))
    axis = [component / gluon_norm for component in incoming_gluon_spatial]
    ...
    local_value = float(local_point["projected"]["copied_s07_projected_local_matrix"])
    ...
    output = {
        "stage": "HggMadGraphAzimuthalAverageValidation-v1",
        "status": "complete",
