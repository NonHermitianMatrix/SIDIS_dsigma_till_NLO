...
assert[
  AssociationQ[evaluator] && evaluator["Status"] === "Complete" &&
    evaluator["Stage"] === "HggFourDimensionalEvaluator-v1" &&
...
assert[
  AssociationQ[metadata] && metadata["status"] === "complete" &&
    metadata["stage"] === "HggFiniteCutMadGraphSample-v1" &&
...
assert[
  Max[Abs[Im[Flatten[unitProjectionValues]]]] === 0.0,
  "compiled evaluator returned a nonzero imaginary component"
];
unitProjectionValues = Re[unitProjectionValues];
...
assert[! FileExistsQ[outputCSVTemporary], "stale S11 CSV temporary exists"];
...
checks = <|
  ...
  "physical_representative_charge_not_reapplied" -> True,
...
|>;
summary = <|
  "stage" -> "HggFiniteCutPointwiseComparison-v1",
  "status" -> "complete",
  ...
  "normalization" -> <|
    "s09_physical_representative_consumed" -> True,
...
assert[! FileExistsQ[outputJSONTemporary], "stale S11 JSON temporary exists"];
