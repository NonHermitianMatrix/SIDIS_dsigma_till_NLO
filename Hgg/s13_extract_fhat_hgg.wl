...
(*
  Hgg stage S13: convert the two finite S12 projector contractions into
  ...
  projector action. This stage sets epsilon to zero only in these compact
  weights, combines coefficient pairs fieldwise, and rebuilds one symbolic
  ordinary-integral action for each hatted structure function.
*)
...
s12SHA256 = FileHash[s12Path, "SHA256"];
...
assert[Sort[Keys[s12["FiniteHattedHardFunctionsByProjector"]]] ===
    Sort[projectors],
  "s12_result does not contain both completed projector hard functions."];

s12Sources = s12["SourceResults"];
assert[AssociationQ[s12Sources] &&
    FileExistsQ[s12Sources["EndpointResolvedReal"]] &&
    FileHash[s12Sources["EndpointResolvedReal"], "SHA256"] ===
      s12Sources["EndpointResolvedRealSHA256"] &&
    FileExistsQ[s12Sources["CollinearCounterterms"]] &&
    FileHash[s12Sources["CollinearCounterterms"], "SHA256"] ===
  ...
  "An S12 upstream source or paper SHA-256 binding is invalid."];
...
assert[FreeQ[finiteProjectorPairs, epsilon | _SeriesData],
  "An S12 finite projector coefficient contains epsilon or SeriesData."];
...
assert[AssociationQ[physicalMapping] &&
    KeyExistsQ[physicalMapping, "XHat"] &&
    KeyExistsQ[physicalMapping, "S23Upper"],
...
assert[FreeQ[{xHat, integrationInterval}, epsilon],
  "The S12 physical map unexpectedly contains epsilon."];
assert[TrueQ[Cancel[Together[xHat - xB/xi]] === 0],
  "The S12 XHat map is not xB/xi."];
assert[TrueQ[Cancel[Together[
      s23Upper - (Q2 (xi/xB - 1) (1 - zH) - PHT2/zH)
    ]] === 0],
  "The S12 s23 upper limit is not the validated physical limit."];

(*
  Retain only the finite pairs and compact map. Clearing the complete S12
  container before constructing F hats avoids holding duplicate large data.
*)
Clear[s12, physicalMapping];
ClearSystemCache[];
...
Print["S13_STAGE: rebuilding the two finite Hgg structure-function actions"];
...
Print["S13_STAGE: validating exact epsilon-free Hgg F hats"];
functionChecks = AssociationMap[
  Function[structureFunction,
    <|
      "ContainsNoEpsilonOrSeries" -> FreeQ[
      ...
      "RetainsPhysicalFlavorChargeWeight" -> ! FreeQ[
        fHatFunctions[structureFunction],
        HggFlavorChargeSum
      ],
...
s13Checks = <|
  ...
  "EpsilonSetToZero" -> And @@ Table[
    TrueQ[functionChecks[structureFunction]["ContainsNoEpsilonOrSeries"]],
    {structureFunction, structureFunctions}
  ],
  ...
  "PhysicalFlavorChargeWeightRetained" -> And @@ Table[
    TrueQ[
      functionChecks[structureFunction][
        "RetainsPhysicalFlavorChargeWeight"
      ]
    ],
    {structureFunction, structureFunctions}
  ],
...
assert[And @@ Values[s13Checks],
  "At least one final Hgg S13 validation check is not True."];
...
s13Result = <|
  ...
  "Checks" -> s13Checks,
  "MemoryStrategy" ->
    "serial fieldwise Pg/PPP combination with source fields dropped after use; no duplicated subkernel copies",
...
Print["S13_STAGE: writing finite hatted Hgg structure functions"];
...
Print["S13_SUCCESS_FINITE_FHAT_HGG"];
