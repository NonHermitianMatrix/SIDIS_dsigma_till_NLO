...
(*
  ...
  Read the validated finite Hgg F1Hat/F2Hat actions saved by S13 directly,
  ...
  evaluated.  Existing Hgg files are read-only; the only output is compact
...
hggDirectory = DirectoryName[scriptDirectory];
s13Path = FileNameJoin[{hggDirectory, "s13_result"}];
...
accidentalGlobalFeynCalcSymbolQ[s_Symbol] :=
  Context[Unevaluated[s]] === "Global`" &&
    MemberQ[feynCalcOwnedSymbolNames, SymbolName[Unevaluated[s]]];
...
couplingAndColorRules = {
  ...
  Nf -> benchmark["Nf"],
  HggFlavorChargeSum -> benchmark["FlavorChargeSum"]
};
...
assert[
  FileExistsQ[s13["SourceResult"]] &&
    FileHash[s13["SourceResult"], "SHA256"] ===
      s13["SourceResultSHA256"] &&
    FileExistsQ[s13["AuthoritativePaper"]] &&
    FileHash[s13["AuthoritativePaper"], "SHA256"] ===
      s13["AuthoritativePaperSHA256"],
  "s13_result provenance hashes do not match their source artifacts."];
...
assert[feynCalcContextCleanQ[fHatFunctions],
  "s13_result contains accidental Global color or coupling symbols."];
...
Do[
  ...
  assert[TrueQ[pair["Endpoint"] === 0] &&
    ...
    structureFunction <> " is not regular-only as required for BigTMD " <>
      "channel 4."];
...
payload = <|
  ...
  "Construction" -> <|
    ...
    "ReferenceDistributionContent" -> "regular only"
  |>,
  "ComparisonLevel" ->
    "finite Hgg coefficient action before PDFs, FFs, and xi convolution",
  "Conventions" -> <|
    "Couplings" -> "EL=1 and g_s=1",
    "Color" -> "SU(3): CA=3, CF=4/3, TF=1/2",
    "FlavorCount" -> benchmark["Nf"],
    "FlavorChargeSum" -> N[benchmark["FlavorChargeSum"], 17],
    "ActiveFlavorCharges" -> N[{2/3, -1/3, -1/3, 2/3}, 17],
    "BigTMDChannel" -> 4,
    "BigTMDChargeCases" -> {"A"},
