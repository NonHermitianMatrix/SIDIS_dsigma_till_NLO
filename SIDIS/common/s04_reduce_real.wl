...
mapping = sidisGet[sidisPath[{root, "s03_result.wl"}]];
gate["accepted real mapping and geometry identity", TrueQ[mapping["Accepted"]] &&
  mapping["GeometryHash"] === sidisHash[sidisPath[{root, "s02_result.wl"}], "SHA256"] &&
  mapping["SourceHash"] === sidisHash[sidisPath[{root, "s03_real_families.wl"}], "SHA256"]];
...
inputHash = Hash[{sourceHash, sidisHash[sidisPath[{root, "s03_result.wl"}], "SHA256"],
  mapping["GeometryHash"], sidisHash[kira, "SHA256"], sidisHash[fermat, "SHA256"]}, "SHA256"];
work = sidisPath[{root, "common", "s04_kira_real", IntegerString[inputHash, 16]}];
...
put[<|"InputHash" -> inputHash, "SourceHash" -> sourceHash, "Bounds" -> bounds,
  ...
  "CutEnergyConditions" -> geometry["CutEnergyConditions"]|>, sidisPath[{work, "s04_configuration.wl"}]];
...
Export[sidisPath[{work, "s04_kira_stdout.log"}], run["StandardOutput"], "Text"];
Export[sidisPath[{work, "s04_kira_stderr.log"}], run["StandardError"], "Text"];
...
declaredMasters = toCut[ToExpression /@ StringCases[sidisImport[masterFile, "Text"],
...
resultFile = sidisPath[{root, "s04_result.wl"}];
put[<|"Rules" -> rules, "Masters" -> masters, "Targets" -> targets,
  "Bounds" -> bounds, "Families" -> families, "SourceHash" -> sourceHash,
  "InputHash" -> inputHash, "MappingHash" -> sidisHash[sidisPath[{root, "s03_result.wl"}], "SHA256"],
...
Do[put[<|"Channel" -> channel, "RealMappingHash" -> mapping["Channels"][channel]["SHA256"],
  "SharedReduction" -> "../s04_result.wl", "SharedReductionHash" -> sidisHash[resultFile, "SHA256"],
  "Accepted" -> True, "MasterEvaluationPerformed" -> False|>,
  sidisPath[{root, channel, "s04_result.wl"}]], {channel, Keys[mapping["Channels"]]}];
