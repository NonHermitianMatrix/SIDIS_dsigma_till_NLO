...
manifest=Get[FileNameJoin[{root,"s11_result.wl"}]];
regions=Get[FileNameJoin[{root,"s15_result.wl"}]];
...
softRows[coefficient_,piece_,sign_] := Module[
  {power=piece["Power"],integerPower,kappa,expression,series,rows,assumptions},
  ...
  assumptions=branchAssumptions[sign]&&w>0;
  expression=epsilonSeries[(coefficient/.D->4-2eps/.t->sign omega-s)piece["Regular"],1,"soft epsilon expansion"];
  series=bounded[Series[expression,{w,0,-1-integerPower},Assumptions->assumptions],"soft recoil expansion"];
  If[Normal[series]===0,Return[{}]];
  gate["soft recoil expansion is a complete integer Laurent series",
    MatchQ[series,_SeriesData]&&series[[6]]===1&&series[[5]]>-1-integerPower];
  rows=Table[{kappa,n+integerPower,bounded[reduce[SeriesCoefficient[series,n],assumptions],"soft coefficient"]},
    {n,series[[4]],-1-integerPower}];
  If[!FreeQ[rows,w],
    rows=Map[Function[row,ReplacePart[row,3->bounded[FullSimplify[
      PowerExpand[row[[3]]/.Log[arg_]:>Log[Factor[arg]],Assumptions->assumptions],assumptions],
      "physical endpoint logarithms"]]],rows]];
  gate["soft coefficients contain no recoil variable",FreeQ[rows,w]];
  Select[rows,Last[#]=!=0&]];
...
assemble[channel_,key_,component_,channelHash_] := Catch[Module[
  ...
  directory=FileNameJoin[{root,channel,"s17_cache",IntegerString[hash,16]}];
  If[!DirectoryQ[directory],CreateDirectory[directory,CreateIntermediateDirectories->True]];
  file=FileNameJoin[{directory,"s17_result.wl"}];
  If[FileExistsQ[file],saved=Get[file];If[saved["InputHash"]===hash&&TrueQ[saved["Accepted"]],Return[saved]]];
  ...
  branches=Association@Table[
    ...
    put[<|"Rows"->rows,"Combined"->combined,"InputHash"->hash|>,
      FileNameJoin[{directory,"s17_soft_"<>ToString[sign]<>".wl"}]];
    ...
    put[value,FileNameJoin[{directory,"s17_branch_"<>ToString[sign]<>".wl"}]];
    sign->value,{sign,{1,-1}}];
...
If[workers>0,ParallelEvaluate[$HistoryLength=0];DistributeDefinitions[gate,put,bounded,
  branchAssumptions,reduce,epsilonSeries,laurentProduct,softPieces,softRows,assemble,
...
Do[
  ...
  file=FileNameJoin[{root,channel,"s17_result.wl"}];put[result,file];
  AssociateTo[outputs,channel-><|"File"->channel<>"/s17_result.wl","Hash"->FileHash[file,"SHA256"]|>];
...
put[<|"Channels"->outputs,"InputHashes"->inputHashes,"SourceHash"->sourceHash,
  "AcceptedCutDistributions"->True,"FinalHatsAssembled"->False|>,FileNameJoin[{root,"s17_result.wl"}]];
