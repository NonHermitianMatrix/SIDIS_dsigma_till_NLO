...
gate["S02 acceptance/source", TrueQ[geometry["Accepted"]] &&
  geometry["SourceHash"] === FileHash[FileNameJoin[{root, "s02_definitions.wl"}], "SHA256"]];
...
geometryHash = FileHash[FileNameJoin[{root, "s02_result.wl"}], "SHA256"];
...
mapTask[task_] := Catch[Module[{file, saved, value, rows},
  ...
  Print["MAP ", task["Channel"], " ", task["TensorKey"], " ", task["Label"],
    " kernel ", $KernelID];
  gate["pair file remains unchanged", FileHash[task["InputFile"], "SHA256"] === task["FileHash"]];
  value = Get[task["InputFile"]][task["Selector"]] /. task["CoordinateRules"];
...
Do[
  channelDirectory = FileNameJoin[{root, channel}];
  inputDirectory = FileNameJoin[{channelDirectory, "s01_inputs"}];
  channelFile = FileNameJoin[{channelDirectory, "s02_result.wl"}];
  gate[channel <> " S02 identity", FileHash[channelFile, "SHA256"] ===
    geometry["Channels"][channel]["SHA256"]];
  data = Get[channelFile]; keys = Keys[data["Contractions"]];
  coordinates = data["CoordinateRules"];
  importManifest = Import[FileNameJoin[{channelDirectory, "s01_result.json"}], "RawJSON"];
  importedHashes = Association[(#["path"] -> FromDigits[#["sha256"], 16]) & /@ importManifest["files"]];
  inputHash = Hash[{sourceHash, geometryHash, FileHash[channelFile, "SHA256"]}, "SHA256"];
  cacheDirectory = FileNameJoin[{channelDirectory, "s03_cache", IntegerString[inputHash, 16]}];
  ...
  If[MemberQ[{"Hqq", "Hqg", "Hgq"}, channel],
    ...
    files = Sort[FileNames[If[channel === "Hqq", {"*_Pg_*.wl", "*_Ppp_*.wl"},
      {"Pg_*.wl", "Ppp_*.wl"}], FileNameJoin[{inputDirectory, cacheStage}], Infinity]],
    files = Select[Sort[FileNames["real_*.wl", FileNameJoin[{inputDirectory, "s02_result"}]]],
      StringMatchQ[FileBaseName[#], RegularExpression["real_[0-9]+"]] &]];
  gate[channel <> " unintegrated pair files exist", Length[files] > 0];
  Do[
    relativeFile = StringDrop[file, StringLength[root] + 1];
    fileHash = FileHash[file, "SHA256"];
    ...
    Do[
      ...
      AppendTo[tasks, <|"Channel" -> channel, "TensorKey" -> key, "Label" -> label,
        ...
        "CacheFile" -> FileNameJoin[{cacheDirectory, label <> ".wl"}]|>], {selector, selectors}],
    {file, files}];
  ...
  put[<|"Channel" -> channel, "Coefficients" -> coefficients, "Targets" -> targets,
    ...
    FileNameJoin[{channelDirectory, "s03_result.wl"}]];
  AssociateTo[channelResults, channel -> <|"File" -> channel <> "/s03_result.wl",
    "SHA256" -> FileHash[FileNameJoin[{channelDirectory, "s03_result.wl"}], "SHA256"],
...
Export[FileNameJoin[{root, "s03_targets"}], StringRiffle[targetText /@ allTargets, "\n"] <> "\n", "Text"];
put[<|"Channels" -> channelResults, "Targets" -> allTargets,
  "GeometryHash" -> geometryHash, "SourceHash" -> sourceHash,
  "ParallelRuntime" -> runtime, "Accepted" -> True|>, FileNameJoin[{root, "s03_result.wl"}]];
