...
generateSelf[field_,exclusions_,label_] := Module[{topologies,diagrams,raw,amplitudes,fields,masses,assignedFields},
  ...
  If[label==="one_flavor",assignedFields=Cases[diagrams,HoldPattern[Rule[Field[_],value_]]:>value,Infinity];
    ...
    put[<|"Diagrams"->diagrams,"MeasuredFlavors"->fields|>,FileNameJoin[{work,"s14_flavor_labels.wl"}]];
...
selfProjection[data_,type_] := Module[{amplitude,free,insertion,denominator,indices,projector,ratio,longitudinal=0,routes,candidates,valid,routing,coordinateDenominator,originalRatio},
  ...
  put[<|"Original"->originalRatio,"RoutingCandidates"->candidates,"ValidRoutings"->valid|>,
    FileNameJoin[{work,"s14_routing_"<>IntegerString[Hash[originalRatio,"SHA256"],16]<>".wl"}]];
...
previousSourceFile=FileNameJoin[{root,"s14_previous_14692901","s14_uv_residues.wl"}];
previousText=Import[previousSourceFile,"Text"];currentText=Import[$InputFileName,"Text"];
...
previousHash=Hash[{FileHash[previousSourceFile,"SHA256"],FileHash[FileNameJoin[{root,"s05_result.wl"}],"SHA256"],
  FileHash[FileNameJoin[{root,"s02_result.wl"}],"SHA256"]},"SHA256"];
previousWork=FileNameJoin[{root,"common","s14_uv",IntegerString[previousHash,16]}];
...
Do[
  ...
  Do[
    ...
    previousFile=FileNameJoin[{previousWork,"s14_"<>label<>".wl"}];
    saved=If[FileExistsQ[file],Get[file],If[FileExistsQ[previousFile],
      inherited=Get[previousFile];gate[label<>" inherited UV map identity",inherited["InputHash"]===previousHash];
