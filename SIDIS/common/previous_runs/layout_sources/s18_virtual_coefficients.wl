...
manifest=Get[FileNameJoin[{root,"s05_result.wl"}]];
...
collect[channel_,mode_,diagrams_,channelHash_] := Catch[Module[
  ...
  directory=FileNameJoin[{root,channel,"s18_cache",IntegerString[hash,16]}];
  If[!DirectoryQ[directory],CreateDirectory[directory,CreateIntermediateDirectories->True]];
  file=FileNameJoin[{directory,"s18_result.wl"}];
...
Do[
  file=FileNameJoin[{root,channel,"s05_result.wl"}];data=Get[file];hash=FileHash[file,"SHA256"];
  ...
  file=FileNameJoin[{root,channel,"s18_result.wl"}];put[result,file];
  AssociateTo[outputs,channel-><|"File"->channel<>"/s18_result.wl","Hash"->FileHash[file,"SHA256"]|>];
...
put[<|"ActualCoefficientOrders"->requiredOrders,"PlannedOrders"->Map[#["RequiredOrder"]&,parents["VirtualMasterParents"]]|>,
  FileNameJoin[{root,"s18_required_orders.wl"}]];
...
put[<|"Channels"->outputs,"RequiredOrders"->requiredOrders,"SourceHash"->sourceHash,
  "InputHashes"->inputHashes,"Accepted"->True|>,FileNameJoin[{root,"s18_result.wl"}]];
