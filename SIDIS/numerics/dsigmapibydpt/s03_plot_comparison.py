...
def record(message):
    line = "SIDIS numerics S03 " + datetime.now(timezone.utc).isoformat() + ": " + message
...
def main():
    ...
    # The reference keeps its original coefficient provenance. Only physical
    # inputs are shared: this consumer intentionally uses different F-hat files.
    maps_path=ROOT.parent/'s04_result'; pdf_path=ROOT.parent/'s06_result'
    old_maps_path=ROOT.parent/'references/original_s04_result.json'
    old_pdf_path=ROOT.parent/'references/original_s06_result.json'
    maps, pdf, old_maps, old_pdf=[json.loads(p.read_text()) for p in
        [maps_path,pdf_path,old_maps_path,old_pdf_path]]
    assert sha(old_maps_path)==reference['shared_numerical_inputs']['s04_result']
    assert sha(old_pdf_path)==reference['shared_numerical_inputs']['s06_result']
    for name in ['s04_result','s06_result','dsigmapibydpt/s01_result']:
        assert theory['input_hashes'][name]==sha(ROOT.parent/name)
    shared_checks={
        'experimental_data':sha(ROOT/'s01_result')==reference['shared_numerical_inputs']['dsigmapibydpt/s01_result'],
        **{key:maps[key]==old_maps[key] for key in
           ['defining_physics_inputs','expressions','c_expressions','tensor_projector_inverse']},
        **{key:pdf[key]==old_pdf[key] for key in
           ['PDF','FFs','flavor_order','raw_pdf_order','alpha_EM','GeV_minus2_to_pb']},
        'vendor_inputs':{Path(k).name:v for k,v in pdf['input_hashes'].items()}==
                        {Path(k).name:v for k,v in old_pdf['input_hashes'].items()},
        'alpha_prescription':pdf['alpha']['prescription']==old_pdf['alpha']['prescription'],
        'alpha_parameters':all(np.isclose(pdf['alpha'][k],v,rtol=1e-12,atol=1e-14)
                               for k,v in old_pdf['alpha'].items() if isinstance(v,(int,float))),
        'alpha_samples':all(np.isclose(pdf['alpha']['samples'][k],v,rtol=1e-12,atol=1e-14)
                            for k,v in old_pdf['alpha']['samples'].items())}
    assert all(shared_checks.values()),shared_checks
    inputs += [maps_path,pdf_path,old_maps_path,old_pdf_path]
    ...
    fig.text(.13, .035, 'MRST2002 NLO; shared FFs, scales and H1 forward-pion cuts. Theory errors: integration only.\n'
             'Hgq input; unresolved MadGraph real comparison retained; uniform azimuth.\n'
    ...
    result = dict(stage='s03', status='Complete', producer_sha256=sha(Path(__file__)),
        ...
        shared_physical_settings_checks=shared_checks,
        fhat_input_sha256=theory['input_hashes']['s01_result'],
