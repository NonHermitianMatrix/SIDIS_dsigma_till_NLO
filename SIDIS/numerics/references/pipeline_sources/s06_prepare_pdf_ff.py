...
def record(message):
    with (ROOT.parent/'progress.md').open('a') as f:
        f.write('\nNumerics S06 '+datetime.now(timezone.utc).isoformat()+': '+message+'\n')
...
def main():
    ...
    mg=ROOT.parent/'Hqqbar/madgraph_check/software/MG5_aMC_v3_7_0'
    for name in ['mrst2002.f','jeppe02.f']:
        shutil.copyfile(mg/'Template/NLO/Source/PDF'/name,mrst/name)
    shutil.copyfile(mg/'Template/NLO/lib/Pdfdata/mrst2002nlo.dat',mrst/'mrst2002nlo.dat')
    ...
    smparams=(mg/'models/sm/parameters.py').read_text()
...
'''
    ...
    command=['gfortran','-shared','-fPIC','-O2','-std=legacy','-ffixed-line-length-none',
    ...
    inputs=[p for p in [archive,mg/'models/sm/parameters.py',*ephox.iterdir(),*mrst.iterdir()] if p.is_file()]
    result=dict(stage='s06',status='Complete',producer_sha256=sha(Path(__file__)),
        ...
        # A barn is defined as 10^-28 m^2; this is a unit definition.
        GeV_minus2_to_pb=(constants.hbar*constants.c/constants.giga/constants.electron_volt)**2/(constants.pico*10.**-28),
