...
def record(message):
    line = "SIDIS numerics S08 " + datetime.now(timezone.utc).isoformat() + ": " + message
...
def main():
    ...
    names=['Hqq','Hgg','Hqqbar','Hqqprime','Hgq','Hqg']
    ...
    contract=contracts['Hgq']
    model=ROOT/'vendor/model/particles.py'
    reuse=json.loads((ROOT/'s01_reuse_manifest.json').read_text())
    assert sha(model)==reuse['CommonInputs']['vendor/model/particles.py']['SHA256']
    model_charges={}
    ...
    for active_names in [['u','d','s','c'],['u','d','s','c','b']]:
        ...
        lum=dict(Hgg=sum(charge[i]**(2*contracts["Hgg"]["ChargeParameterDegree"]) for i in quarks)*f[0]*d[0],
            Hgq=f[0]*sum(charge[i]**contracts["Hgq"]["ChargeDegree"]*d[i] for i in active),
            Hqg=d[0]*sum(charge[i]**contracts["Hqg"]["ChargeDegree"]*f[i] for i in active),
            Hqqbar=sum(charge[i]**(2*contracts["Hqqbar"]["ChargeParameterDegree"])*f[i]*d[order.index(species[i] if 'bar' in order[i] else species[i]+'bar')] for i in active))
        moments=[]
        charge_values={S.Symbol('charge'+str(i+1)):model_charges[name] for i,name in enumerate(active_names)}
        for row in contracts['Hqq']['FlavorMoments'][str(len(quarks))]:
        ...
        for label,key in [('IncomingChargeSquared','eq2'),('PrimeChargeSquared','eqp2'),('MixedIncomingPrimeCharge','eq_eqp')]:
            monomial=S.sympify(contracts['Hqqprime']['ChargeMonomials'][key])
            lum['Hqqprime_'+label]=sum(monomial.subs({eq:charge[i],eqp:charge[j]})*f[i]*d[j] for i,j in pairs)
    ...
    actions={}
    plus_arguments={ch:S.sympify(contracts[ch]['PlusLogArgument'],locals={'ss':ss,'B':B}) for ch in ['Hqq','Hqg','Hgq']}
