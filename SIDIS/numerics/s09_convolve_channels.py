...
def record(message):
    line = "SIDIS numerics S09 " + datetime.now(timezone.utc).isoformat() + ": " + message
...
def derive_and_build():
    ...
    lum_labels = ['Hqq_group0', 'Hqq_group1', 'Hgg', 'Hqqbar',
                  ...
                  'Hqqprime_MixedIncomingPrimeCharge', 'Hgq', 'Hqg']
    ...
    build = subprocess.run([os.environ.get('CXX','g++'), '-std=c++20', '-O3', '-shared', '-fPIC', str(path),
    ...
    contract = dict(stage='s09', status='IntegrandBuilt', producer_sha256=sha(Path(__file__)),
        ...
        caveats=[physical['azimuth'], 'Hgq retains its documented unresolved MadGraph real-emission comparison; supplied coefficients are unchanged.',
...
def prepare_reuse(folder):
    ...
    unchanged=[c for c in CHANNELS if c!='Hgq']
...
def integrate_bin(task):
    ...
    evaluator = Evaluator(spec['bounds'],['Hgq'] if baseline else None)
    ...
    for level in range(refinements+1):
        ...
        record_value = dict(**spec, status='Complete' if all(gates.values()) else 'ConvergenceNotAccepted',
            ...
            integrated_channels=['Hgq'] if baseline else CHANNELS,
