...
def record(message):
    line = "SIDIS numerics S07 " + datetime.now(timezone.utc).isoformat() + ": " + message
...
def main():
    ...
    parser.add_argument('--channels',nargs='+',default=['Hqq','Hgg','Hqqbar','Hqqprime','Hgq','Hqg'])
    ...
    derive=subprocess.run([os.environ.get('WOLFRAM_KERNEL','/home/physics/wolframengine/opt/Wolfram/WolframEngine/15.0/Executables/WolframKernel'),'-noinit','-noprompt','-run',code],capture_output=True,text=True,timeout=30)
    ...
    for spec in specs:
        ...
        assert spec['FhatInputSHA256']==payloads[channel]
    ...
    run=subprocess.run([os.environ.get('CXX','g++'),'-std=c++20','-O3','-shared','-fPIC',str(source),'-o',str(libpath)],capture_output=True,text=True,timeout=60)
