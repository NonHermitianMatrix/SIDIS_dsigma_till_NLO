<|"InputHash" -> 
  524731851600252029320150748515633058493619995660498299994419083897517769401\
83, "Function" -> Log[(4*Q^2*(-1/4*(-omega^2 + radius^2)/Q^2 + s)*
     (-omega + (-omega^2 + radius^2)/(4*Q^2) + s))/
    ((-omega^2 + radius^2)*(omega + Q^2 - (-omega^2 + radius^2)/(4*Q^2)))], 
 "Assumptions" -> Q^2 > 0 && s > (-omega^2 + radius^2)/(4*Q^2) && 
   (-omega^2 + radius^2)/(4*Q^2) > 0 && 
   -Q^2 + (-omega^2 + radius^2)/(4*Q^2) - s < omega - s && 
   omega - s < -1/4*(-omega^2 + radius^2)/s && mu > 0 && B > 0 && Q > 0 && 
   omega > 0 && Element[nf, Integers] && nf >= 1 && alphaS > 0 && 
   Element[eq | otherChargeMoment[1] | otherChargeMoment[2], Reals] && 
   radius > 0, "Value" -> -Log[omega + 2*Q^2 - radius] - 
   Log[-omega + radius] - Log[omega + radius] - Log[omega + 2*Q^2 + radius] + 
   Log[omega^2 - radius^2 + 4*Q^2*s] + 
   Log[-omega^2 - 4*omega*Q^2 + radius^2 + 4*Q^2*s]|>
