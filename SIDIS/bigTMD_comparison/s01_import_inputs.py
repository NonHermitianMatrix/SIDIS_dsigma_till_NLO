...
def production_file(channel, name):
    layout_file = SIDIS / 'common/s21_result/layout.json'
    if layout_file.is_file():
        layout = json.loads(layout_file.read_text())
        original = channel + '/' + name
        if original in layout['Moves']:
            return SIDIS / layout['Moves'][original]
    base = SIDIS / channel
...
def main():
    existing = ROOT / 's01_result.json'
    if existing.is_file():
        saved = json.loads(existing.read_text())
        assert saved['Accepted']
        for channel, entries in saved['Channels'].items():
            assert sha(production_file(channel, 's20_result.wl')) == entries['production.wl']['SHA256'], channel
            for name, entry in entries.items():
                target = ROOT / channel / 's01_result'
                if name in ('production.wl', 'born.wl', 'subtraction.wl') or name.startswith('reference/'):
                    target = target / name
                else:
                    target = target / 'reference' / name
                assert sha(target) == entry['SHA256'], str(target)
        for name, entry in saved['Helpers'].items():
            assert sha(ROOT / 'reference' / name) == entry['SHA256'], name
        print('S01_SUCCESS: verified the unchanged frozen comparison inputs at the current layout.')
        return
    records = {}
