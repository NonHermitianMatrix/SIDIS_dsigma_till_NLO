...
(* The unpolarized quark average uses the paper's two spin states. *)
initialAverages = <|"Hgq" -> Factor[1/(gluonSpinCount adjointDimension)],
  "AuxHqq" -> Factor[1/(2 fundamentalDimension)]|>;
...
bornContraction[label_, mode_] := Module[{entry, amplitude, a, b, gl, reference, value, finalSpecies, weight},
  entry = If[label === "Hgq", "Born", "AuxHqqBorn"];
  amplitude = Total[generated[entry]] /. {SMP["e"] -> 1, SMP["g_s"] -> 1};
  {gl, reference} = If[label === "Hgq", {p, k1}, {k2, p}];
  ...
  weight = Count[finalSpecies, generated["ObservedSpecies"]]/Times @@ (Factorial /@ Values[Counts[finalSpecies]]);
...
bornResults = Association@Table[label -> Association@Table[
  ...
  {mode, {"Pg", "Ppp", "PhotonWard", "GluonWard"}}], {label, {"Hgq", "AuxHqq"}}];
...
result = <|"BornPg" -> bornResults["Hgq"]["Pg"], "BornPpp" -> bornResults["Hgq"]["Ppp"],
  "AuxHqqBorn" -> KeyTake[bornResults["AuxHqq"], {"Pg", "Ppp"}], "BornChecks" -> bornResults,
  ...
  "GluonSpinCount" -> gluonSpinCount, "InitialAverages" -> initialAverages,
...
Print["Hgq Born Pg = ", InputForm[result["BornPg"]]];
Print["Hgq Born Ppp = ", InputForm[result["BornPpp"]]];
