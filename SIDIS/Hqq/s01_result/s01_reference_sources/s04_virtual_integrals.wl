...
If[workerCount > 0,
  ...
  ParallelEvaluate[$HistoryLength = 0; Global`$FeynCalcStartupMessages = False;
    FeynCalc`$FeynHelpersLoadInterfaces = {"PackageX"}; Global`$LoadAddOns = {"FeynHelpers"};
    Get["FeynCalc`"]];
  DistributeDefinitions[paxMeasure];
  workerBubbles = ParallelEvaluate[FCClearScalarProducts[]; SPD[r, r] = -Q2;
    PaXEvaluateUVIRSplit[FAD[ell, ell + r], ell,
      PaXImplicitPrefactor -> paxMeasure, PaXC0Expand -> True,
      PaXD0Expand -> True, PaXAnalytic -> True] /. ScaleMu -> mu];
  gate["every worker reproduces the accepted loop backend",
    And @@ (Factor[# - backend] === 0 & /@ workerBubbles)];
